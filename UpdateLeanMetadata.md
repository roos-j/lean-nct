...
If the status is not "ToDo", the do the following:
  - If Lean names are listed in Status.md for that label, then make sure there is exactly one \lean{..} command after the label is defined in the tex file which must contain the exact Lean names as a comma-separated list in the argument (i.e. add it if its not there).
