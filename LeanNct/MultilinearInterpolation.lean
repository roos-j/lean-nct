...
This file is an adaption of a formalization of Stein's analytic interpolation theorem.
...
theorem aux_multilinear_input_deformation_properties
  ...
  · intro z i
    ...
    simp [aux_multilinear_input_deformation]
  constructor
  · intro t i
    dsimp only [aux_multilinear_input_deformation]
    apply eLpNorm_input_deformation_left_coeff_ennreal_le_one
...
private theorem aux_multilinear_pairing_growth
  ...
  intro z
  have hnorm :
      ‖∫ y, B (Fin.cons f (fun i ↦ aux_multilinear_input_deformation p₀ p₁ p g z i)) y * h y ∂ν‖ ≤ C := by
    rw [congrFun hF (z : ℂ)]
    calc
      ...
      _ = C := rfl
  calc
    ‖∫ y, B (Fin.cons f (fun i ↦ aux_multilinear_input_deformation p₀ p₁ p g z i)) y * h y ∂ν‖ ≤ C := hnorm
...
theorem multilinear_interpolation_two_endpoint_normalized
  ...
  have hBanalytic : ∀ (f : SimpleFunc X ℂ) (h : SimpleFunc Y ℂ),
      ...
      DiffContOnCl ℂ (fun z ↦ ∫ y,
        Bcur f (fun i ↦ aux_multilinear_input_deformation p₀ p₁ p (fun i ↦ g i.succ) z i) y * h y ∂ν)
        (verticalStrip 0 1) := by
    intro f h hf hh
    simpa only [Bcur, MultilinearMap.curryLeft_apply] using
