...
module
public import LeanNct.Auto.MainArgument.SandwichKernel
...
noncomputable def squareRootGaussianDifference (a : ℤ → ℝ)
    ...
    ℝ → ℝ := fun x =>
  ∫ ξ : ℝ,
    Real.sqrt (Auto.gaussian (a (j - 1) * ξ) -
      Auto.gaussian (a j * ξ)) *
...
theorem lMultiplierAtScale_memDoubleSequence {n : ℕ} (γ : GeometricParameters n)
  ...
  have hconv := Auto.aux_memW0_convolutionAlong
...
theorem aux_memW0_shear_difference
  ...
  have hPs : MemW0 (P ∘ Auto.aux_convolutionAlongShear (t, t)) :=
    Auto.aux_memW0_comp_continuousLinearEquiv hP _
  have hsub := Auto.aux_memW0_sub hPs hP
  convert hsub using 1
  funext z
  rcases z with ⟨v, q⟩
  dsimp [P, Function.comp_def, Auto.aux_convolutionAlongShear]
  ring
...
theorem aux_translation_difference_bound
  ...
  have hf_int : Integrable f :=
    Auto.aux_memW0_integrable_of_addHaar hf
...
theorem aux_lMultiplierAtScale_scaled_convolution_difference
  ...
  have hA : Integrable (fun q : ℝ => f (v - q • (t, t)) * standardBump q) := by
    ...
    have hslice_int := Auto.aux_memW0_integrable_of_addHaar hslice
...
theorem aux_lMultiplierAtScale_error_le
  ...
  have hconv_int : Integrable (fun v : RealPlane => ∫ p : ℝ,
    ...
    have hconv' : Integrable (fun v : RealPlane => ∫ p : ℝ,
      ...
      Auto.aux_memW0_integrable_of_addHaar hconv
    convert hconv' using 1
    funext v
    rcases v with ⟨v₁, v₂⟩
    simp [smul_eq_mul, sub_eq_add_neg]
...
theorem aux_fourierPlane_diagonalConvolution
  ...
  have hcomplex := aux_fourierPlaneComplex_diagonalConvolution
    ...
    (Complex.continuous_ofReal.comp hF.1)
      (Auto.aux_memW0_integrable_of_addHaar hF).ofReal
    (Complex.continuous_ofReal.comp hρ.1)
      (Auto.aux_memW0_integrable_of_addHaar hρ).ofReal ξ
...
theorem aux_fourierReal_sub (f g : ℝ → ℝ)
  ...
  have he_bound : ∀ x, ‖e x‖ ≤ (1 : ℝ) := by
    ...
    rw [show e x = Complex.exp
      ...
      Complex.norm_exp]
    norm_num
  have hfint := Auto.aux_memW0_integrable_of_addHaar hf
  have hgint := Auto.aux_memW0_integrable_of_addHaar hg
...
theorem aux_fourierPlane_twoDimensionalGaussian_one (q : Fin 2 → ℝ)
    ...
    aux_fourierPlane (twoDimensionalGaussian q 1) (ξ, η) =
      (Auto.gaussian (q 0 * ((ξ + η) / Real.sqrt 2)) *
        Auto.gaussian (q 1 * ((-ξ + η) / Real.sqrt 2)) : ℂ) := by
...
theorem aux_gaussian_mul_diag (a b x : ℝ) :
    (Auto.gaussian (a * x) *
      Auto.gaussian (b * (-x)) : ℂ) =
      (Auto.gaussian (Real.sqrt (a ^ 2 + b ^ 2) * x) : ℂ) := by
  norm_cast
...
theorem aux_gaussian_wOne_diag (a x : ℝ) :
    (Auto.gaussian
      (a * ((-x + -x) / Real.sqrt 2)) : ℂ) =
      (Auto.gaussian (Real.sqrt 2 * a * x) : ℂ) := by
...
theorem aux_fourierReal_diagonalSlice (f : RealPlane → ℝ) (hf : MemW0 f)
  ...
  have hS : MemW0 S := by
    ...
    exact Auto.aux_memW0_comp_continuousLinearEquiv hf
      aux_diagonalShear
  have hSint : Integrable S :=
    Auto.aux_memW0_integrable_of_addHaar hS
...
theorem hMultiplier_vanishing_integral {n : ℕ} (γ : GeometricParameters n)
  ...
  have hS : MemW0 (fun q : RealPlane => hMultiplier γ i j (aux_diagonalShear q)) :=
    Auto.aux_memW0_comp_continuousLinearEquiv hH
      aux_diagonalShear
  have hL : MemW0 L := by
...
theorem aux_lMultiplierAtScale_largeScaleConvolution_bound
  ...
  have hFswap : MemW0 Fswap := by
    have hs := Auto.aux_memW0_comp_continuousLinearEquiv hF
  ...
  have hinner (q : ℝ) :
    ...
    have hFx : Integrable (fun x : ℝ => ‖F (x, q)‖) :=
      (Auto.aux_memW0_integrable_of_addHaar hFxmem).norm
...
theorem aux_lMultiplierAtScale_tendsto_integral_norm_atTop
  ...
  have hF : MemW0 F := by
    ...
    exact Auto.aux_memW0_comp_continuousLinearEquiv hH
      aux_diagonalShear
...
theorem aux_hMultiplier_diagonal_translate_integrable
  ...
  have hS : MemW0 S := by
    exact Auto.aux_memW0_comp_continuousLinearEquiv
  ...
  have hGint : Integrable (fun q : ℝ => hMultiplier γ i j ((v.1 - v.2) + q, q)) :=
    Auto.aux_memW0_integrable_of_addHaar hG
...
theorem aux_multiplierIndexPartialSum_memW0 {n : ℕ} (γ : GeometricParameters n)
  ...
  have hsum : MemW0 (fun x : RealPlane => ∑ ξ ∈ aux_multiplierIndexTruncation γ N,
    ...
    apply Auto.aux_memW0_finset_sum
    intro ξ hξ
...
noncomputable def mKernelMultiplierIndexPartialSum {n : ℕ}
    ...
    (Mι : MultiplierIndex γ → Auto.MKernel γ.k) (N : ℕ) :
    Auto.MKernel γ.k := by
  classical
  exact ∑ ξ ∈ aux_multiplierIndexTruncation γ N,
...
theorem aux_prismForm_finset_sum {n k : ℕ} (hk : 1 ≤ k) (hkn : k ≤ n)
    {α : Type*} (s : Finset α) (M : α → Auto.MKernel k)
    (hM : ∀ a ∈ s, MemW0 (M a))
    (F : Fin n → SchwartzMap (Auto.RealVector n) ℝ) :
    Auto.prismForm n k hk hkn (fun y => ∑ a ∈ s, M a y)
        (fun i => F i) =
      ∑ a ∈ s, Auto.prismForm n k hk hkn (M a) (fun i => F i) := by
  classical
  let e : Fin (Fintype.card {a // a ∈ s}) ≃ {a // a ∈ s} :=
    (Fintype.equivFin {a // a ∈ s}).symm
  let M' : Fin (Fintype.card {a // a ∈ s}) → Auto.MKernel k :=
    fun q => M (e q).1
  ...
  have hsumM (y : Auto.RealVector k ×
      Auto.RealVector k) :
      (∑ q, M' q y) = ∑ a ∈ s, M a y := by
    simpa [M'] using hsum (fun a => M a y)
  have hsumP :
      (∑ q, Auto.prismBrascampLiebForm n k hk hkn
        (Auto.mToK k hk (M' q)) (fun i x => F i x)) =
        ∑ a ∈ s, Auto.prismBrascampLiebForm n k hk hkn
          (Auto.mToK k hk (M a)) (fun i x => F i x) := by
    simpa [M'] using hsum (fun a =>
      Auto.prismBrascampLiebForm n k hk hkn
        (Auto.mToK k hk (M a)) (fun i x => F i x))
  calc
    Auto.prismForm n k hk hkn (fun y => ∑ a ∈ s, M a y)
        (fun i => F i) =
        Auto.prismBrascampLiebForm n k hk hkn
          (Auto.mToK k hk (fun y => ∑ q, M' q y))
          (fun i x => F i x) := by
      congr 3
      funext y
      exact (hsumM y).symm
    _ = Auto.prismBrascampLiebForm n k hk hkn
        (fun z => ∑ q, Auto.mToK k hk (M' q) z)
        (fun i x => F i x) := by
      rw [Auto.aux_mToK_finset_sum k
        (Fintype.card {a // a ∈ s}) hk M' hM']
    _ = ∑ q, Auto.prismBrascampLiebForm n k hk hkn
        (Auto.mToK k hk (M' q)) (fun i x => F i x) := by
      apply Auto.aux_prismBrascampLiebForm_finset_sum
      intro q
      exact Auto.mToK_memW0 n k hk hkn (M' q) (hM' q)
    _ = ∑ a ∈ s, Auto.prismBrascampLiebForm n k hk hkn
        (Auto.mToK k hk (M a)) (fun i x => F i x) := hsumP
    _ = ∑ a ∈ s, Auto.prismForm n k hk hkn (M a)
        (fun i => F i) := by rfl
...
noncomputable def prismMultiplierIndexPartialAbsoluteSum {n : ℕ}
    (γ : GeometricParameters n)
    (Mι : MultiplierIndex γ → Auto.MKernel γ.k)
    (F : Fin n → SchwartzMap (Auto.RealVector n) ℝ) (N : ℕ) :
    ℝ≥0∞ := by
  classical
  exact ∑ ξ ∈ aux_multiplierIndexTruncation γ N,
    if hξ : ξ ∈ multiplierIndexSet γ then
      ‖Auto.prismForm n γ.k γ.one_le_k γ.k_le_n
        (Mι ⟨ξ, hξ⟩) (fun i => F i)‖ₑ
    else 0
...
theorem aux_mToK_sub {k : ℕ} (hk : 1 ≤ k)
    ...
    Auto.mToK k hk (fun y => M₁ y - M₂ y) =
      fun z => Auto.mToK k hk M₁ z -
        Auto.mToK k hk M₂ z := by
  funext z
  unfold Auto.mToK
  rw [integral_sub]
  · exact Auto.aux_memW0_integrable_of_addHaar
      (Auto.mToK_integrand_memW0 k k hk le_rfl M₁ hM₁ z)
  · exact Auto.aux_memW0_integrable_of_addHaar
      (Auto.mToK_integrand_memW0 k k hk le_rfl M₂ hM₂ z)
...
theorem aux_prismForm_sub {n k : ℕ} (hk : 1 ≤ k) (hkn : k ≤ n)
    ...
    (hM₁ : MemW0 M₁) (hM₂ : MemW0 M₂)
    (F : Fin n → SchwartzMap (Auto.RealVector n) ℝ) :
    Auto.prismForm n k hk hkn M₁ (fun i => F i) -
      Auto.prismForm n k hk hkn M₂ (fun i => F i) =
      Auto.prismForm n k hk hkn (fun y => M₁ y - M₂ y)
        (fun i => F i) := by
  unfold Auto.prismForm
  rw [aux_mToK_sub hk M₁ M₂ hM₁ hM₂]
  exact Auto.aux_prismBrascampLiebForm_sub n k hk hkn
    (Auto.mToK k hk M₁)
    (Auto.mToK k hk M₂)
    (Auto.mToK_memW0 n k hk hkn M₁ hM₁)
    (Auto.mToK_memW0 n k hk hkn M₂ hM₂) F
...
theorem prismSumLeSumPrismL1 {n : ℕ} (γ : GeometricParameters n)
    ...
    (hconverges : Tendsto (fun N : ℕ =>
      ...
      atTop (nhds 0))
    (F : Fin n → SchwartzMap (Auto.RealVector n) ℝ)
    (hF : F ∈ Auto.normalizedFunctionTuples n) :
    ‖Auto.prismForm n γ.k γ.one_le_k γ.k_le_n M
        (fun i => F i)‖ₑ ≤
  ...
  have hdiff : MemW0 (P - M) :=
    Auto.aux_memW0_sub hP hM
  have hform_sub :
      Auto.prismForm n γ.k γ.one_le_k γ.k_le_n P
          (fun i => F i) -
        Auto.prismForm n γ.k γ.one_le_k γ.k_le_n M
          (fun i => F i) =
        Auto.prismForm n γ.k γ.one_le_k γ.k_le_n (P - M)
          (fun i => F i) := by
    exact aux_prismForm_sub γ.one_le_k γ.k_le_n P M hP hM F
  have herror :
      ‖Auto.prismForm n γ.k γ.one_le_k γ.k_le_n (P - M)
          (fun i => F i)‖ₑ ≤ eLpNorm (P - M) 1 volume := by
    change ‖Auto.prismBrascampLiebForm n γ.k γ.one_le_k γ.k_le_n
        (Auto.mToK γ.k γ.one_le_k (P - M))
          (fun i x => F i x)‖ₑ ≤ _
    exact
      (Auto.prismBLInequality n γ.k γ.one_le_k γ.k_le_n
        (Auto.mToK γ.k γ.one_le_k (P - M))
        (Auto.mToK_memW0 n γ.k γ.one_le_k γ.k_le_n
          (P - M) hdiff) F hF).trans
      (Auto.mToK_eLpNorm_one_le n γ.k γ.one_le_k γ.k_le_n
        (P - M) hdiff)
  have hpartial :
      ‖Auto.prismForm n γ.k γ.one_le_k γ.k_le_n P
  ...
  calc
    ‖Auto.prismForm n γ.k γ.one_le_k γ.k_le_n M
        ...
        ‖Auto.prismForm n γ.k γ.one_le_k γ.k_le_n P
          ...
          congr 1
          ring
    _ ≤ ‖Auto.prismForm n γ.k γ.one_le_k γ.k_le_n P
            (fun i => F i)‖ₑ +
          ‖Auto.prismForm n γ.k γ.one_le_k γ.k_le_n (P - M)
...
theorem aux_fourScaleGaussianRhoFrequency_real
  ...
  refine ⟨(a - b) * Real.rpow
    (Auto.gaussian (muMinus * xi) -
      Auto.gaussian (muPlus * xi)) nu, ?_⟩
  unfold fourScaleGaussianRhoFrequency
  rw [hfa, hfb]
  norm_cast
...
theorem aux_fourierReal_sigmaMultiplier_vertical {n : ℕ}
    ...
    aux_fourierReal (sigmaMultiplier γ ι i j) u =
      (Real.sqrt (Auto.gaussian
        (γ.scales i 1 (j + ι.1.2 - 1) * u) -
        Auto.gaussian (γ.scales i 1 (j + ι.1.2) * u)) : ℂ) := by
...
theorem aux_diagonalConvolution_assoc
  ...
  have hH_w0 : MemW0 H := by
    have hPT : MemW0 (P ∘ T) :=
      Auto.aux_memW0_comp_continuousLinearEquiv hP T
    have hPTA : MemW0 ((P ∘ T) ∘ assoc) :=
      Auto.aux_memW0_comp_continuousLinearEquiv hPT assoc
  ...
  have hH : Integrable H :=
    Auto.aux_memW0_integrable_of_addHaar hH_w0
  have hLeft :
      (∫ p : ℝ, (∫ q : ℝ, F (v.1 - p - q, v.2 - p - q) * rho q) * sigma p) =
        ∫ p : ℝ, ∫ q : ℝ, H (q, p) := by
    apply integral_congr_ae
