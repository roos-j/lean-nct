...
description: Synchronize label-adjacent `\\lean{...}` and `\\leanok` commands in `blueprint/blueprint.tex` from the formalization statuses in `automation/Status.md`. Use when asked to update, refresh, or reconcile Lean metadata in a repository containing `automation/Status.md`, `automation/Instructions.md`, and `blueprint/`.
---
...
Run from the repository root. Confirm that `automation/Status.md`, `automation/Instructions.md`, and `blueprint/` exist. Treat existing working-tree changes as user-owned: inspect the relevant diffs and avoid changing unrelated files.

Before editing the blueprint, make sure `automation/Status.md` reflects the current formalization state according to `automation/Instructions.md`. Do not infer a status from existing LaTeX metadata or from Lean declarations. If `automation/Status.md` is stale, malformed, or inconsistent and the discrepancy cannot be resolved confidently, stop and create a diagnostic report instead of guessing.
...
If that file does not exist, stop; do not modify `automation/Status.md` or create a metadata report without an input blueprint.

Record the source path and treat a missing source label as a diagnostic issue.

### 2. Parse and match status entries

Read every entry in `automation/Status.md` with the form:

```text
\\label{manuscript label}: [Status] (Lean: leanName, anotherLeanName) (date and time)
```

Use the exact label text between braces for matching. Do not match by substring, theorem title, or Lean name. Exclude `aux_` names from the Lean-name list; if they appear in `automation/Status.md`, record the inconsistency and do not silently invent a replacement.
...
- For `Todo`, add neither command, even if Lean names are listed in `automation/Status.md`.
- For a non-`Todo` status with one or more Lean names, add exactly one `\\lean{...}` command containing the exact names from `automation/Status.md` as a comma-separated list. Preserve names verbatim and do not add `aux_` names.
...
Re-read the edited blueprint and verify every `automation/Status.md` entry:
...
Create a report only when an issue was encountered. Write it in `automation/` as `automation/lean-metadata-diagnostic-YYMMDD-HHmm.md` using the current local date and time. Include:
