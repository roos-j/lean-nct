...
\begin{center}
    ...
    This is the document of PASS 3.}}
\end{center}
...
\begin{theorem}[Main ergodic theorem \auto]\label{thm:ergodicthm}
...
\end{equation}
where \jr{GPT: Please inline these constant definitions (no displays), and also in the rest of the manuscript: if there are more than one line defining constants, then inline them}
for $n=2$,
\begin{equation}\label{main ergodic theorem constant}
C_{\ref{thm:ergodicthm},2,r}
=C_{\ref{thm:ergodicthmJ},2}^{1/2},
\end{equation}
and, for $n\ge3$,
\begin{equation}\label{auto:main-ergodic-theorem-constant-higher-rank}
C_{\ref{thm:ergodicthm},n,r}
=2^{1-2^{1-n}}\left(\tfrac{r}{r-2^{n-1}}\right)^{1/r}C_{\ref{thm:ergodicthmJ},n}^{1/2}.
\end{equation}
\end{theorem}
...
\begin{theorem}[Main twisted theorem]\label{thm:nct main real}\jr{GPT: this theorem is lacking \using{..} annotations, add them. Also make sure every theorem in the document has these annotations. To be clear, we only annotate the theorems that are directly used in the proof, we do not add indirect dependencies through theorems that are used.}
...
C_{\ref{thm:nct main real}}=2^{769}.
\end{equation}
\end{theorem}
...
\begin{proposition}[Permutation of the endpoint estimate]\label{permutation of endpoint estimate}\using{thm:nct main real}
...
\begin{exttheorem}[Multilinear interpolation]\label{multilinear interpolation external}
...
\begin{proposition}[Interpolated real-variable estimate]\label{interpolated real variable estimate}\using{permutation of endpoint estimate}\using{exponent polytope}\using{multilinear interpolation external}
...
\begin{exttheorem}[Calder\'on transference principle]\label{Calderon transference external}
...
\begin{theorem}[Ergodic jump estimate with general exponents]\label{thm:ergodicthm-interpolation}\using{interpolated real variable estimate}\using{Calderon transference external}
...
\begin{theorem}[Symmetric ergodic jump estimate]\label{thm:ergodicthmJ}\using{thm:ergodicthm-interpolation}
...
\begin{proposition}\label{W_0 fiber integrals}\using{P:lp-embedding}\using{W_0 radius independence}
...
\begin{proof}[Proof \auto]
Choose a linear map $\tau:\R^{n-m-l}\to\R^n$ such that
\[
...
\begin{proposition}\label{W_0 Brascamp Lieb}\using{tensor Wiener}\using{W_0 fiber integrals}
...
|f(y)|\le 2^NC(1+|x|)^{-N}.
\]
The right-hand side is integrable in $x$, so $f\in W_0(\R^n)$.
\end{proof}
...
\label{convolution vector}\using{W_0 Brascamp Lieb}
...
\begin{proposition}[singly cancellative Cauchy-Schwarz]\label{single cancellative Cauchy-Schwarz}\using{cube BL inequality}
...
\resizebox{0.98\textwidth}{!}{$\displaystyle \sum_{j\in[J)} \int_{(\R^2)^k}\int_{\R^{n-k+1}} \rho_j(y^1-y^0,\Sigma(y^0)+\Sigma(x_{[k-1,n)})) \times \Big( \int_\R \Big( \prod_{\substack{h\in[2)^k\\ i\in[k,n)}} F_{i}(y^h,r,x_{[k,n)\setminus i}) \Big) \phi_j(r-x_{k-1})\,dr \Big) \times \Big( \prod_{h\in[2)^k}F_{k-1}(y^h,x_{[k,n)}) \Big) dx_{[k-1,n)}\,dy\, .$}
\end{equation}
...
The first term $\left< A , A \right>$ is no greater than\jr{GPT: Earlier I said, don't use multiline equations. You responded by resizing long equations such as in the following display. This is not what was intended. What I meant was: don't use multiline display environments. If an equation becomes too long you can split the display at a reasonable point into multiple normal displays using begin{equation} or square bracket commands. Resizing formulas to make an equation fit in the line is not permissible. Refactor all uses of that accordingly}
\begin{equation}
\resizebox{0.98\textwidth}{!}{$\displaystyle \sum_{j\in[J)}\int_{(\R^2)^k}\int_{\R^{n-k+1}} |\rho_j|(y^1-y^0,\Sigma(y^0)+\Sigma(x_{[k-1,n)})) \times \Big| \int_\R \Big( \prod_{\substack{h\in[2)^k\\ i\in[k,n)}} F_{i}(y^h,r,x_{[k,n)\setminus i}) \Big) \phi_j(r-x_{k-1})\,dr \Big|^2 dx_{[k-1,n)}\,dy,$}
\end{equation}
...
\begin{equation}
\resizebox{0.98\textwidth}{!}{$\displaystyle \sum_{j\in[J)}\int_{(\R^2)^{k+1}}\int_{\R^{n-k}} \int_\R |\rho_j|((y^1-y^0)_{[k)},\Sigma(y^0_{[k)})+x_{k-1}+\Sigma(x_{[k,n)})) \times \Big( \prod_{\substack{h\in[2)^{k+1}\\ i\in[k,n)}} F_{i}(y^h,x_{[k,n)\setminus i}) \Big) \phi_j(y_k^1-x_{k-1})\phi_j(y_k^0-x_{k-1}) \,dx_{k-1}\,dx_{[k,n)}\,dy$}
\end{equation}
...
\resizebox{0.98\textwidth}{!}{$\displaystyle \sum_{j\in[J)} \int_{(\R^2)^{n-1}} \int_{\R^2} \rho_j(y^1-y^0, \Sigma(y^0)+\Sigma(x_{[n-2,n)})) \times \Big( \int_{\R} \Big(\prod_{h\in [2)^{n-1}} F_{n-1}(y^h,r)\Big) \phi_j(r-x_{n-2})\,dr\Big) \times \Big( \int_{\R} \Big(\prod_{h\in [2)^{n-1}} F_{n-2}(y^h,s)\Big) \phi_j(s-x_{n-1})\,ds\Big) dx_{[n-2,n)}\,dy .$}
\end{equation}
...
\begin{equation}
\resizebox{0.98\textwidth}{!}{$\displaystyle \sum_{j\in[J)} \int_{(\R^2)^{n-1}} \int_{\R^2} |\rho_j|(y^1-y^0, \Sigma(y^0)+\Sigma(x_{[n-2,n)})) \times \Big| \int_{\R} \Big(\prod_{h\in [2)^{n-1}} F_{n-1}(y^h,r)\Big) \phi_j(r-x_{n-2})\,dr \Big|^2 dx_{[n-2,n)}\,dy .$}
\end{equation}
Using that the $F_i$ are real and renaming the integration variables $r$, which appear twice
through the square, once $y_{n-1}^0$ and once $y_{n-1}^1$, this is equal to
\begin{equation}
\resizebox{0.98\textwidth}{!}{$\displaystyle \sum_{j\in[J)} \int_{(\R^2)^n}\int_{\R^2} |\rho_j|((y^1-y^0)_{[n-1)},\Sigma(y^0_{[n-1)})+x_{n-2}+x_{n-1}) \times \Big(\prod_{h\in[2)^n}F_{n-1}(y^h)\Big) \phi_j(y_{n-1}^1-x_{n-2})\phi_j(y_{n-1}^0-x_{n-2}) \,dx_{n-2}\,dx_{n-1}\,dy .$}
\end{equation}
...
\begin{equation}
\resizebox{0.98\textwidth}{!}{$\displaystyle \sum_{j\in[J)} \int_{(\R^2)^{n-1}} \int_{\R^2} |\rho_j|(y^1-y^0, \Sigma(y^0)+\Sigma(x_{[n-2,n)})) \times \Big| \int_{\R} \Big(\prod_{h\in [2)^{n-1}} F_{n-2}(y^h,s)\Big) \phi_j(s-x_{n-1})\,ds \Big|^2 dx_{[n-2,n)}\,dy .$}
\end{equation}
...
\begin{equation}
\resizebox{0.98\textwidth}{!}{$\displaystyle \sum_{j\in[J)} \int_{(\R^2)^n}\int_{\R^2} |\rho_j|((y^1-y^0)_{[n-1)},\Sigma(y^0_{[n-1)})+x_{n-2}+x_{n-1}) \times \Big(\prod_{h\in[2)^n}F_{n-2}(y^h)\Big) \phi_j(y_{n-1}^1-x_{n-1})\phi_j(y_{n-1}^0-x_{n-1}) \,dx_{n-2}\,dx_{n-1}\,dy .$}
\end{equation}
...
We have
\[
\resizebox{0.98\textwidth}{!}{$\displaystyle \Theta_{k+1}(K)(\F) = \int_{(\R^2)^{k+1}}\int_{\R^{n-k}}\int_\R \rho((y^1-y^0)_{[k+1)\setminus i},\Sigma(y^0)+\Sigma(x_{[k,n)})-q) \times \phi(y_i^1-y_i^0+q)\phi(q) \prod_{\substack{h\in [2)^{k+1}\\j\in[k,n)}} F_j(y^h,x_{[k,n)\setminus j}) \,dq\,dx_{[k,n)}\,dy .$}
\]

Substituting \(q\) by \(y_i^0-q\) and integrating first in \(y_i^0,y_i^1\), this is equal to
\[
\resizebox{0.98\textwidth}{!}{$\displaystyle \int_{(\R^2)^{k}}\int_{\R^{n-k}}\int_\R \rho((y^1-y^0)_{[k+1)\setminus i}, \Sigma(y^0_{[k+1)\setminus i})+q+\Sigma(x_{[k,n)})) \times \Big| \int_\R \Big( \prod_{\substack{h\in [2)^{k+1}\\ h_i=0\\ j\in[k,n)}} F_j(y^h,x_{[k,n)\setminus j}) \Big) \phi(y_i^0-q)\,dy_i^0 \Big|^2 \,dq\,dx_{[k,n)}\,dy_{[k+1)\setminus i},$}
\]
...
\begin{proposition}[$M$ to $K$]\label{M to K}\using{W_0 Brascamp Lieb}
...
\begin{proposition}[Positivity]\label{Positivity M}\using{tensor Wiener}\using{Positivity K}
...
\begin{proposition}[Cauchy-Schwarz at $n-1$]\label{Cauchy-Schwarz at n-1}\using{doubly cancellative Cauchy-Schwarz}\using{Monotonicity K}
...
|\g^{(m)}(x)|
\le
\bigl(38(m+1)\bigr)^{m/2}e^{-\pi x^2/2}.
\]
...
\begin{proposition}[Elementary Gaussian properties]\label{Elementary Gaussian properties}\using{Gaussian bump decay}
...
\[
\int_\R \bigl(2\pi\g(\xi)+12\pi^2\xi^2\g(\xi)+\pi E(\xi)\bigr)\,d\xi = 2\pi+12\pi^2\tfrac{1}{2\pi}+\pi\tfrac{2}{\alpha} = 8\pi+2\sqrt{\pi} \le30.
\]
Therefore
\[
    \|B''\|_1\le 26+30=56.
...
\begin{proposition}[square root of Gaussian decay]\label{square root of Gaussian decay}\using{auxiliary function B}\using{poisson to abel}\using{Elementary Gaussian properties}
For $x\in\R$ let
...
\[
    ...
    \tfrac{100}{4\pi^2x^2}.
\]
If $|x|\le1$, then $8\le32\langle x\rangle^2$. If $|x|>1$, then $|x|^{-2}\le4\langle x\rangle^2$. Thus
\begin{equation}\label{eq:Bcheck-decay}
|\widecheck B(x)|\le32\langle x\rangle^2.
\end{equation}
The explicit formula for $\widecheck{E_\alpha}$ similarly gives
\[
0\le\widecheck{E_\alpha}(x)\le8\langle x\rangle^2.
\]
Combining these estimates, we obtain\jr{GPT: In the following line we are for no good reason estimating $40$ by a $100$. The correct thing to do is to instead try to change the final constant to $40$, similar with other explicit constants everywhere in the text. Check if they can be improved 'for free' or with minor modifications to the argument.}
\[
|\rho(x)|\le40\langle x\rangle^2\le100\langle x\rangle^2.
\]
...
Where do you apply this and not \ref{lem:smoothdecay2}}
...
\begin{equation}\label{auto:mean-value-bump-bound} |\rho(x+y)-\rho(x)| \le  2^{N+1}\max_{0\le \nu\le N}\|(2\pi)^{-\nu}\widehat{\rho}^{(\nu)}\|_\infty \min(1, 2\pi |y|) (\langle x+y\rangle^N + \langle x\rangle^N). \end{equation}
...
\begin{proposition}[diagonal square root]\label{diagonal square root}\using{square root of Gaussian decay}\using{Gaussian bump decay}\using{two bump estimate}
...
\begin{lemma}[constant $C_{\ref{diagonal square root},N}$ \auto]\label{constant diagonal square root}\using{Gaussian bump decay}\using{two bump estimate}\using{square root of Gaussian decay}
For every $N\ge1$,
...
C_{\ref{diagonal square root},2}\le2^{15}.
\end{equation}
\end{lemma}
\begin{proof}[Proof \auto]
...
=19200<2^{15}.
\]
Substitution in the defining maximum proves the first estimate. At $N=2$, the defining expression is $19200\sqrt2<2^{15}$.
\end{proof}


\begin{proposition}[derivative of diagonal square root]\label{derivative of diagonal square root}\using{square root of Gaussian decay}\using{Elementary Gaussian properties}\using{Gaussian bump decay}\using{two bump estimate}
...
\begin{lemma}[constant $C_{\ref{derivative of diagonal square root},N}$ \auto]\label{constant derivative diagonal square root}\using{Gaussian bump decay}\using{two bump estimate}\using{derivative of diagonal square root}
For every $N\ge1$,
...
C_{\ref{derivative of diagonal square root},2}\le2^{16}.
\end{equation}
\end{lemma}
\begin{proof}[Proof \auto]
By \eqref{auto:Gaussian-bump-decay-constant},
...
C_{\ref{Gaussian bump decay},0,2}=12,
\qquad
C_{\ref{Gaussian bump decay},1,2}=12\sqrt{76}<105,
\qquad
C_{\ref{two bump estimate},2,2}=16.
\]
The product of these three constants is less than $20160<2^{15}$. Substitution in the defining maximum proves the first estimate, and at $N=2$ it gives $C_{\ref{derivative of diagonal square root},2}<40320<2^{16}$.
\end{proof}
...
\begin{lemma}\label{L:gaussian-bump-estimate}\using{L:gaussian-estimate}
...
\begin{lemma}[constant $C_{\ref{L:gaussian-bump-estimate},N}$ \auto]\label{constant gaussian bump estimate}\using{L:gaussian-estimate}\using{L:gaussian-bump-estimate}
For every $N\in\N$,
\begin{equation}\label{constant gaussian bump estimate bound}
C_{\ref{L:gaussian-bump-estimate},N}\le2^{5(N+1)^2}.
\end{equation}
For $0\le N\le3$,
\begin{equation}\label{auto:constant-gaussian-bump-estimate-small}
C_{\ref{L:gaussian-bump-estimate},N}\le2^6.
\end{equation}
\end{lemma}
\begin{proof}[Proof \auto]
For $0\le l\le N$, the defining formula in Lemma \ref{L:gaussian-estimate}, together with $e^\pi<2^5$, $\pi<2^2$, and $l!\le (N+1)^N$, gives
\[
C_{\ref{L:gaussian-estimate},l}\le2^{4(N+1)^2}.
\]
The binomial coefficients in the definition of $C_{\ref{L:gaussian-bump-estimate},N}$ sum to $2^N$, and $(2\pi)^{-N}\le1$, which proves the first estimate. Direct substitution for $N=0,1,2,3$ gives values less than $49$, proving the second estimate.\jr{GPT: Another example of constant waste. When aboslute constants are $<1000$ you don't have to use a power of $2$, instead use the smaller number}
\end{proof}
...
\begin{lemma}\label{L:faa-di-bruno}\using{L:derivative-estimate-for-G}\using{Gaussian bump decay}
...
\begin{lemma}[constant $C_{\ref{L:faa-di-bruno},N}$ \auto]\label{constant faa di bruno}\using{L:derivative-estimate-for-G}\using{Gaussian bump decay}
For every $N\in\N$,
...
C_{\ref{L:faa-di-bruno},0}\le2^6,
\quad
C_{\ref{L:faa-di-bruno},1}\le2^{14},
\quad
C_{\ref{L:faa-di-bruno},2}\le2^{32},
\quad
C_{\ref{L:faa-di-bruno},3}\le2^{56}.
\end{equation}
\end{lemma}
\begin{proof}[Proof \auto]
Since $\pi>3$ and the first four terms of the exponential series give $e^{9/16}>7/4$, we have $1-e^{-3\pi/16}>3/7$. Thus Lemma \ref{L:derivative-estimate-for-G} gives
\[
C_{\ref{L:derivative-estimate-for-G},q}
\le q!\left(\tfrac73\right)^{q+1}.
\]
Together with \eqref{auto:Gaussian-bump-decay-constant}, the fact that a partition of $[N)$ has at most $N$ blocks, and the bound $N^N$ for the number of partitions, this proves the first estimate by substitution in the defining sum. For $N=0,1,2,3$, separating the finitely many partitions according to their block sizes gives respectively the bounds $2^6,2^{14},2^{32},2^{56}$.
\end{proof}


\begin{lemma}\label{L:second-gaussian-estimate}\using{L:gaussian-bump-estimate}\using{L:faa-di-bruno}
...
\begin{lemma}[constant $C_{\ref{L:second-gaussian-estimate},N}$ \auto]\label{constant second gaussian estimate}\using{L:gaussian-bump-estimate}\using{L:faa-di-bruno}
For every $N\in\N$,
...
C_{\ref{L:second-gaussian-estimate},0}\le2^{14},
\quad
C_{\ref{L:second-gaussian-estimate},1}\le2^{20},
\quad
C_{\ref{L:second-gaussian-estimate},2}\le2^{35},
\quad
C_{\ref{L:second-gaussian-estimate},3}\le2^{57}.
\end{equation}
\end{lemma}
\begin{proof}[Proof \auto]
Substitute the estimates from the defining formulas of Lemmas \ref{L:gaussian-bump-estimate} and \ref{L:faa-di-bruno} into the defining sum. Bounding the $N+1$ summands and the binomial coefficients gives the first estimate. For $N=0,1,2,3$, use $\pi>3$, $\pi<22/7$, and $e^\pi<24$ in the same finite sum; direct substitution gives the displayed four bounds.
\end{proof}
...
\begin{lemma}\label{L:gaussian-bump-decomposition}\using{L:gaussian-estimate}\using{L:second-gaussian-estimate}
...
\begin{proposition}[four scale Gaussian kernel estimate]\label{four scale Gaussian kernel}\using{L:gaussian-bump-decomposition}\using{lem:smoothdecay2}\using{L:gaussian-bump-estimate}\using{lem:smoothdecay}\using{lem: min and bracket}\using{L:second-gaussian-estimate}
...
\begin{lemma}[constant $C_{\ref{four scale Gaussian kernel},N}$ \auto]\label{constant four scale Gaussian kernel}\using{lem:smoothdecay2}\using{L:gaussian-bump-estimate}\using{L:second-gaussian-estimate}\using{constant gaussian bump estimate}\using{constant second gaussian estimate}
For every $N\in\N$,
...
C_{\ref{four scale Gaussian kernel},2}\le2^{38},
\qquad
C_{\ref{four scale Gaussian kernel},3}\le2^{61}.
\end{equation}
\end{lemma}
\begin{proof}[Proof \auto]
Substitute $C_{\ref{lem:smoothdecay2},N}=2^N$ and the defining estimates from Lemmas \ref{L:gaussian-bump-estimate} and \ref{L:second-gaussian-estimate}. This proves the first estimate. For $N=2,3$, the bounds in \eqref{auto:constant-gaussian-bump-estimate-small} and \eqref{auto:constant-second-gaussian-estimate-small}, substituted in the defining sum, give respectively $2^{38}$ and $2^{61}$.
\end{proof}


\begin{proposition}[mean value four scale Gaussian kernel estimate]\label{mean four scale Gaussian kernel}\using{L:gaussian-bump-decomposition}\using{mean value bump estimate 2}\using{L:gaussian-bump-estimate}\using{L:second-gaussian-estimate}
Let $c>0$, $N\in \mathbb N$ with $N\geq 2$ and assume that $\phi$ is a $W_0(\R)$ function such that
$\widehat{\phi}$ is supported in $[-1,1]$ and equal to $1$ on $[-1/2,1/2]$ and $\widehat{\phi}$ is $N$ times continuously differentiable and $\max_{0\leq m \leq N} \|\widehat{\phi}^{(m)}\|_{\infty} \leq c$.
...
\end{equation}
where
\begin{equation}\label{auto:mean-four-scale-Gaussian-constant} C_{\ref{mean four scale Gaussian kernel},N} =  5\cdot 2^{N+1}\max_{0\leq l \leq N} C_{\ref{L:gaussian-bump-estimate},l} +2^{N+3}\max_{0\leq l \leq N} 2^lC_{\ref{L:second-gaussian-estimate},l}.\end{equation}
...
\begin{equation*}
     |\mathcal{F}^{-1}(\varrho_2)(x+y)-\mathcal{F}^{-1}(\varrho_2)(x)|\le c 2^{N+3}\max_{0\leq l \leq N} 2^lC_{\ref{L:second-gaussian-estimate},l}
     \min(1,2\pi|y|) (\langle x+y\rangle^N+
\langle x\rangle^N).
\end{equation*}
...
\begin{lemma}[constant $C_{\ref{mean four scale Gaussian kernel},N}$ \auto]\label{constant mean four scale Gaussian kernel}\using{L:gaussian-bump-estimate}\using{L:second-gaussian-estimate}\using{constant gaussian bump estimate}\using{constant second gaussian estimate}
For every $N\in\N$,
...
C_{\ref{mean four scale Gaussian kernel},2}\le2^{43}.
\end{equation}
\end{lemma}
\begin{proof}[Proof \auto]
Substitution of the defining estimates from Lemmas \ref{L:gaussian-bump-estimate} and \ref{L:second-gaussian-estimate} proves the first bound. For $N=2$, use \eqref{auto:constant-gaussian-bump-estimate-small} and \eqref{auto:constant-second-gaussian-estimate-small} in the defining maximum to obtain the stated power of two.
\end{proof}
...
\begin{proposition}[telescoping terms]\label{telescoping terms}\using{P:schwartz-into-wiener}\using{prism BL inequality}\using{M to K}
...
\begin{proposition}[positive terms]\label{positive terms}\using{Positivity M}
...
\begin{proposition}\label{square root Gaussian difference W0}\using{diagonal square root}\using{Operations on spaced sequences}
...
\begin{proposition}\label{H-in-X}\using{tensor Wiener}\using{telescoping terms}\using{square root Gaussian difference W0}
...
\begin{proposition}[H vanishing]\label{H vanishing}\using{Elementary Gaussian properties}\using{square root Gaussian difference W0}
...
\begin{proposition}[H vanishing integral]\label{H vanishing integral}\using{H vanishing}
...
\begin{lemma}\label{L:F_t}\using{P:schwartz-into-wiener}\using{convolution vector}\using{H-in-X}
...
\begin{lemma}\label{L:ft-infty}\using{H vanishing integral}
...
\begin{proposition}[Properties of L multipliers]\label{sum L multiplier convergence-L1}\using{P:schwartz-into-wiener}\using{convolution vector}\using{H-in-X}\using{L:F_t}\using{L:ft-infty}
...
\begin{lemma}\label{prism sum le sum prism-L1}\using{prism BL inequality}\using{M to K}
Let $\gamma=(k,u,a)\in \Gamma$.
...
\begin{proposition}\label{auto:N-kernel-well-definedness}\using{four scale Gaussian kernel}\using{H-in-X}\using{convolution vector}
...
\begin{proposition}[Kernel estimate for Gaussian domination]\label{H kernel estimate Gaussian domination}\using{Operations on spaced sequences}\using{Properties of distance of sequences}\using{diagonal square root}\using{Gaussian bump decay}
...
\begin{lemma}[constant $C_{\ref{H kernel estimate Gaussian domination}}$ \auto]\label{constant H kernel estimate Gaussian domination}\using{constant diagonal square root}\using{diagonal square root}\using{Gaussian bump decay}\using{H kernel estimate Gaussian domination}
\begin{equation}\label{constant H kernel estimate Gaussian domination bound}
C_{\ref{H kernel estimate Gaussian domination}}\le2^{33}.
\end{equation}
\end{lemma}
\begin{proof}[Proof \auto]
By \eqref{auto:constant-diagonal-square-root-two},
$C_{\ref{diagonal square root},2}\le2^{15}$, while
$C_{\ref{Gaussian bump decay},0,2}=12$. Substitution in the defining sum gives
\[
C_{\ref{H kernel estimate Gaussian domination}}
\le4\cdot2^{30}+12^2<2^{33}.
\]
\end{proof}


\begin{proposition}[Kernel derivative estimate for Gaussian domination]\label{H kernel derivative estimate Gaussian domination}\using{H kernel estimate Gaussian domination}\using{diagonal square root}\using{derivative of diagonal square root}\using{Gaussian bump decay}
Let $\gamma=(k,u,a)\in \Gamma$ and let $i\in [k)$.
...
\begin{lemma}[constant $C_{\ref{H kernel derivative estimate Gaussian domination}}$ \auto]\label{constant H kernel derivative estimate Gaussian domination}\using{constant diagonal square root}\using{constant derivative diagonal square root}\using{diagonal square root}\using{derivative of diagonal square root}\using{Gaussian bump decay}
\begin{equation}\label{constant H kernel derivative estimate Gaussian domination bound}
C_{\ref{H kernel derivative estimate Gaussian domination}}\le2^{34}.
\end{equation}
\end{lemma}
\begin{proof}[Proof \auto]
By \eqref{auto:constant-diagonal-square-root-two} and \eqref{auto:constant-derivative-diagonal-square-root-two},
\[
C_{\ref{diagonal square root},2}\le2^{15},
\qquad
C_{\ref{derivative of diagonal square root},2}\le2^{16}.
\]
Also $C_{\ref{Gaussian bump decay},0,2}=12$ and
$C_{\ref{Gaussian bump decay},1,2}<105$. Hence the defining sum is less than
\[
4\cdot2^{15}2^{16}+2\cdot12\cdot105<2^{34}.
\]
\end{proof}


\begin{proposition}[Gaussian domination, combined]\label{Gaussian domination combined}\jr{GPT: Put the following three constant definitions in one line, similar elsewhere}
Set
\begin{equation}\label{Gaussian domination combined constants}
C_{\ref{Gaussian domination combined},0}=28.
\end{equation}
Moreover,
\begin{equation}\label{auto:Gaussian-domination-combined-distance-constant}
C_{\ref{Gaussian domination combined},1}=2,
\end{equation}
and
\begin{equation}\label{auto:Gaussian-domination-combined-majorant-constant}
...
The proof of this proposition is split into three cases: Propositions \ref{Gauss domination case 1}, \ref{Gauss domination case 2}, and \ref{Gauss domination case 3}, below.

\begin{proposition}\label{Gauss domination case 1}\using{standard bump properties}\using{mean four scale Gaussian kernel}\using{H kernel estimate Gaussian domination}\using{two bump estimate}\using{bump triangle}\using{Operations on spaced sequences}\using{H vanishing integral}\using{Properties of distance of sequences}\using{orthogonal decay}\using{Gaussian domination}
...
\begin{proposition}[Gaussian domination, far case]\label{Gauss domination case 2}\using{standard bump properties}\using{four scale Gaussian kernel}\using{H kernel derivative estimate Gaussian domination}\using{bump triangle}\using{two bump estimate}\using{Operations on spaced sequences}\using{orthogonal decay}\using{H kernel estimate Gaussian domination}\using{Properties of distance of sequences}\using{Gaussian domination}
The conclusion of Proposition \ref{Gaussian domination combined} holds in the case $\iota=(h,0)$, $h<0$, with the constant in \eqref{Gaussian domination main estimate} equal to
...
\begin{proposition}[Gaussian domination, intermediate case]\label{Gauss domination case 3}\using{standard bump properties}\using{four scale Gaussian kernel}\using{H kernel estimate Gaussian domination}\using{bump triangle}\using{two bump estimate}\using{Operations on spaced sequences}\using{orthogonal decay}\using{Properties of distance of sequences}\using{Gaussian domination}
...
\begin{lemma}[constant $C_{\ref{Gaussian domination combined},2}$ \auto]\label{Gauss domination constant}\using{standard bump properties}\using{constant four scale Gaussian kernel}\using{constant mean four scale Gaussian kernel}\using{four scale Gaussian kernel}\using{mean four scale Gaussian kernel}\using{constant H kernel estimate Gaussian domination}\using{constant H kernel derivative estimate Gaussian domination}\using{H kernel estimate Gaussian domination}\using{H kernel derivative estimate Gaussian domination}\using{two bump estimate}\using{Gauss domination case 1}\using{Gauss domination case 2}\using{Gauss domination case 3}
We have
\begin{equation}\label{auto:Gaussian-domination-case-constant-bounds}
C_{\ref{Gauss domination case 1}}\le2^{120},
\end{equation}
\begin{equation}\label{auto:Gaussian-domination-case-two-constant-bound}
C_{\ref{Gauss domination case 2}}\le2^{153},
\end{equation}
and
\begin{equation}\label{auto:Gaussian-domination-case-three-constant-bound}
C_{\ref{Gauss domination case 3}}\le2^{112}.
\end{equation}
In particular,
...
C_{\ref{two bump estimate},\tfrac32,\tfrac32}<2^5,
\end{equation}
and
\begin{equation}\label{auto:two-bump-small-constant-two-for-Gaussian-domination}
C_{\ref{two bump estimate},2,2}=2^4.
\end{equation}
The bump triangle constant in Proposition \ref{Gauss domination case 1} is $1$, and the other bump triangle constants in the three definitions are at most $2^2$. Since $\pi<2^2$ and $e^{2\pi}<2^{10}$, substitution into \eqref{auto:Gaussian-domination-case-one-constant}, \eqref{auto:Gaussian-domination-case-two-constant}, and \eqref{auto:Gaussian-domination-case-three-constant} gives the three claimed estimates.
\end{proof}
...
\begin{proposition}
\label{vanishing diagonal implies induct positive terms}\using{positive terms}\using{telescoping terms}
Let $k\in\N$ with $1\le k\le n$ and $C\in [1,\infty)$.
...
\begin{proposition}
\label{diagonal band implies vanishing diagonal}\using{sum L multiplier convergence-L1}\using{prism sum le sum prism-L1}\using{sandwich sums L1}
Let $k\in\N$ with $1\le k\le n-1$ and $C\in [1,\infty)$.
...
\begin{proposition}[vanishing kernel integral]\label{vanishing kernel integral}\using{simplification 1 prism}\using{H vanishing integral}
...
\begin{proposition}\label{increase data implies diagonal band}\using{Gaussian domination combined}\using{Cauchy-Schwarz at k}\using{Cauchy-Schwarz at n-1}
...
\begin{proposition}
\label{induct positive terms imply increase data}\using{Gaussian domination combined}\using{Monotonicity K}
Let $k\in\N$ with $1\le k\le n-1$ and $C\in [1,\infty)$.
...
\begin{lemma}[constant $C_{\ref{induct positive terms imply increase data}}$ \auto]\label{constant induct positive terms imply increase data}\using{Gaussian domination combined}
\begin{equation}\label{constant induct positive terms imply increase data bound}
C_{\ref{induct positive terms imply increase data}}\le2^{171}.
\end{equation}
\end{lemma}
\begin{proof}[Proof \auto]
Using \eqref{Gaussian domination combined constants}, \eqref{auto:Gaussian-domination-combined-distance-constant}, and \eqref{auto:Gaussian-domination-combined-majorant-constant}, the defining product is
\[
...
\begin{proposition}\label{P:C_k-induction}\using{vanishing diagonal implies induct positive terms}\using{vanishing kernel integral}\using{induct positive terms imply increase data}\using{increase data implies diagonal band}\using{diagonal band implies vanishing diagonal}
...
\begin{lemma}[constant $C_k$ \auto]\label{P:better-induction}\using{P:C_k-induction}\using{Gaussian domination combined}\using{induct positive terms imply increase data}
...
\begin{theorem}\label{induct positive terms theorem}\using{P:better-induction}\using{P:C_k-induction}
...
\begin{lemma}[constant $C_{\ref{induct positive terms theorem}}$ \auto]\label{constant induct positive terms theorem}\using{induct positive terms imply increase data}\using{Gaussian domination combined}\using{induct positive terms theorem}
\begin{equation}\label{constant induct positive terms theorem bound}
C_{\ref{induct positive terms theorem}}\le2^{358}.
\end{equation}
\end{lemma}
\begin{proof}[Proof \auto]
The defining value of $C_{\ref{induct positive terms imply increase data}}$ is $252\cdot2^{163}$. Hence
\[
2^{17}\bigl(C_{\ref{Gaussian domination combined},2}C_{\ref{induct positive terms imply increase data}}\bigr)^{1/2}
=\sqrt{252}\,2^{175}.
\]
The square of this first summand is $\tfrac{63}{64}2^{358}$, while the remaining two terms in the expanded square are smaller than $2^{181}+2$, and this is smaller than $\tfrac1{128}2^{358}$. Therefore
\[
C_{\ref{induct positive terms theorem}}
<\tfrac{127}{128}2^{358}<2^{358}.
\]
\end{proof}
...
\ls{This lemma should be removed eventually. If there is still any need for it, we can use Propositions~\ref{orthogonal decay} and \ref{bump triangle} instead. (This concerns the proof of Lemma~\ref{lem:leftbump1_long}).}\jr{GPT: Implement Lenka's suggestion here, i.e. replace all uses of this lemma by uses of Propositions~\ref{orthogonal decay} and \ref{bump triangle} (check of course that it works). This will allow us to improve constants further by possibly lowering the value of $N_0$, investigate what we get \cgpt}

\begin{lemma}
\label{lem:transl_pow_v}
    For any $u,v\in \R$ and $N\ge 0$,
    \begin{equation}\label{auto:translated-bracket-bound}\langle u+v\rangle^{N} \le \langle u\rangle^{N}\langle v\rangle^{-N}.\end{equation}
\end{lemma}
...
\begin{lemma} \label{lem:widebump}\using{lem:transl_pow_v}
...
\begin{lemma}\label{lem:thetat_offcenter}\using{orthogonal decay}\using{bump triangle}
Let $k\le-1$. Then for every $v\in\R^2$,
\begin{equation}\label{E:two-terms}
\int_1^2
\langle v_0-t2^{-k}\rangle^4
\langle v_1-t2^{-k}\rangle^4\,dt
\le C_{\ref{lem:thetat_offcenter}}2^k
\langle 2^k(v_0+v_1)\rangle^{3/2}
\langle v_0-v_1\rangle^{3/2},
\end{equation}
where
\begin{equation}\label{off center bump constant}
C_{\ref{lem:thetat_offcenter}}=2^5.
\end{equation}
\end{lemma}
\begin{proof}[Proof \auto]
Put $R=2^{-k}$ and $u=v_0+v_1$. The coordinate comparisons underlying Propositions \ref{orthogonal decay} and \ref{bump triangle} give, more sharply in the present special coordinates, \jr{GPT: Here and in many other places you are saying 'more sharply' or 'sharper' (especially in the recursive constant unfolding lemmas!) and you mean this refering to the version of the previous pass. Keep in mind you are producing a final document that will eventually be publicly available while the previous pass documents are internal only. So the document must not contain any implicit or explicit references to the old document. Fix this!}\jr{GPT: You followed instructions to use Propositions \ref{orthogonal decay} and \ref{bump triangle} here, but you did so in an invalid way: you can never invoke the proof of a proposition, you can only invoke the proposition itself.}
\[
\langle a\rangle^4\langle b\rangle^4
\le
\langle a+b\rangle^2\langle a-b\rangle^2.
\]
Indeed, $1+|a+b|$ and $1+|a-b|$ are each at most $(1+|a|)(1+|b|)$. Thus the left-hand side of \eqref{E:two-terms} is at most
\[
\langle v_0-v_1\rangle^2
\int_1^2\langle u-2tR\rangle^2\,dt
=
2^{k-1}\langle v_0-v_1\rangle^2
\int_{2R}^{4R}\langle u-q\rangle^2\,dq.
\]
If $|u|\le8R$, then
\[
\int_{2R}^{4R}\langle u-q\rangle^2\,dq
\le2
\le2^6\langle u/R\rangle^{3/2}.
\]
If $|u|>8R$, then $q\le|u|/2$ throughout the interval, and hence
\[
\int_{2R}^{4R}\langle u-q\rangle^2\,dq
\le8R\langle u\rangle^2
\le2^4\langle u/R\rangle^{3/2}.
\]
Since $\langle v_0-v_1\rangle^2\le\langle v_0-v_1\rangle^{3/2}$, the result follows.
\end{proof}

\begin{lemma}[constant $C_{\ref{lem:thetat_offcenter}}$ \auto]\label{constant off center bump}
\begin{equation}\label{constant off center bump bound}
C_{\ref{lem:thetat_offcenter}}\le2^5.
\end{equation}
\end{lemma}
\begin{proof}[Proof \auto]
This is the defining value.
\end{proof}
...
C_0=2^{27}.
\end{equation}
A universal pair is a $(C_0,N_0)$-pair.
\end{definition}
...
\begin{lemma}[Existence of a universal pair]\label{lem:cpair}\using{standard bump properties}
...
\le2^{q(q-1)/2+3q}
\]
for $1\le q\le5$. Applying the Fa\`a di Bruno formula to the two trigonometric compositions, using $\pi/2<11/7$, and grouping terms by their block sizes gives the following bounds for derivatives of orders $1,2,3,4,5$:
\[
2^4,
\qquad
2^9,
...
2^{27}.
\]
For completeness, before rounding to powers of two, the corresponding Fa\`a di Bruno sums are
\[
\tfrac{11}{7}2^3,
\]
\[
\tfrac{11}{7}2^7+\left(\tfrac{11}{7}\right)^2 2^6,
\]
\[
\tfrac{11}{7}2^{12}+3\left(\tfrac{11}{7}\right)^2 2^{10}+\left(\tfrac{11}{7}\right)^3 2^9,
\]
\[
\tfrac{11}{7}2^{18}+4\left(\tfrac{11}{7}\right)^2 2^{15}+3\left(\tfrac{11}{7}\right)^2 2^{14}+6\left(\tfrac{11}{7}\right)^3 2^{13}+\left(\tfrac{11}{7}\right)^4 2^{12},
\]
and
\[
\tfrac{11}{7}2^{25}+5\left(\tfrac{11}{7}\right)^2 2^{21}+10\left(\tfrac{11}{7}\right)^2 2^{19}+10\left(\tfrac{11}{7}\right)^3 2^{18}+15\left(\tfrac{11}{7}\right)^3 2^{17}+10\left(\tfrac{11}{7}\right)^4 2^{16}+\left(\tfrac{11}{7}\right)^5 2^{15}.
\]
Direct arithmetic shows that these are smaller than the five displayed powers of two. Thus every derivative of order at most $N_0$ is bounded by $C_0$. The order-zero bound is at most $1$, so $(\phi_0,\phi_1)$ is a $(C_0,N_0)$-pair.
\end{proof}
...
\begin{lemma}[Smoothing decomposition]\label{lem:smoothingdecomp}\using{lem:chardecomp}\using{lem:bumpbasic}
The identity
...
\begin{lemma}\label{lem:theta_decay}\using{lem:smoothdecay2}
Let $1\le N\le N_0$. Then
...
C_{\ref{lem:theta_decay},N}=2^{2N+2}C_0.
\end{equation}
\end{lemma}
\begin{proof}[Proof \auto]
We have
...
\le(1+2^m)C_0\le2^{m+1}C_0.
\]
The support of $\widehat\theta$ has measure at most two. Lemma \ref{lem:smoothdecay2}, applied with $m=N$, gives
\[
|\theta(u)|
\le2^N\,2^{N+1}C_0\,2\langle u\rangle^N
=2^{2N+2}C_0\langle u\rangle^N.
\]
\end{proof}

\begin{lemma}[constant $C_{\ref{lem:theta_decay},N}$ \auto]\label{constant theta decay}\using{lem:theta_decay}
For $1\le N\le N_0$,
\begin{equation}\label{constant theta decay bound}
C_{\ref{lem:theta_decay},N}\le2^{2N+29}.
\end{equation}
\end{lemma}
\begin{proof}[Proof \auto]
Substitute $C_0=2^{27}$ in \eqref{auto:theta-decay-constant-definition}.
\end{proof}
...
\begin{lemma}\label{lem:abs_deriv_ft_phi3_le}\using{lem:ft_phi3_eq}
Let $m\in[N_0+1)$, $k\in\Z$, and $|\xi|\le1$. Then
...
\end{equation}
where \jr{GPT: Please refactor the following display. We do not want any multiline formulas/begin cases. Just state all the cases separately, same in the similar displays below. Also, maybe we don't need 5 derivatives after the constant improvement above, check. \cgpt}
for $m=0$,
...
C_{\ref{lem:abs_deriv_ft_phi3_le},1}=2^5C_0,
\end{equation}
for $m=2$,
\begin{equation}\label{auto:varphi-three-Fourier-derivative-constant-two}
C_{\ref{lem:abs_deriv_ft_phi3_le},2}=2^7C_0^2,
\end{equation}
for $m=3$,
\begin{equation}\label{auto:varphi-three-Fourier-derivative-constant-three}
C_{\ref{lem:abs_deriv_ft_phi3_le},3}=2^{11}C_0^2,
\end{equation}
for $m=4$,
\begin{equation}\label{auto:varphi-three-Fourier-derivative-constant-four}
C_{\ref{lem:abs_deriv_ft_phi3_le},4}=2^{15}C_0^2,
\end{equation}
and, for $m=5$,
\begin{equation}\label{auto:varphi-three-Fourier-derivative-constant-five}
C_{\ref{lem:abs_deriv_ft_phi3_le},5}=2^{19}C_0^2.
\end{equation}
\end{lemma}
\begin{proof}[Proof \auto]
...
\le\pi|\xi|<4.
\]
For $1\le q\le5$, differentiation under the defining integral for $\widehat{\mathbf1_{[0,1)}}$ gives
\[
\left|\tfrac{d^q}{d\xi^q}
\left(2^k\bigl(\widehat{\mathbf1_{[0,1)}}(2^{-k}\xi)-1\bigr)\right)\right|
\le2^{2(q-1)}\tfrac{(2\pi)^q}{q+1}.
\]
For $q=1,2,3,4,5$, respectively, this is bounded by
\[
4,\qquad53,\qquad993,\qquad19950,\qquad417819.
\]
Moreover, the zeroth derivatives of the second and third factors are at most $1$, while for $1\le q\le5$,
\[
\left|\tfrac{d^q}{d\xi^q}\widehat{\phi_0}(2^{-k}\xi)\right|
\le2^{2q}C_0,
\qquad
|\widehat\theta^{(q)}(\xi)|\le(1+2^q)C_0.
\]
Substitution in the three-factor Leibniz rule gives, for $m=0,1,2,3,4,5$, respectively,
\[
4,
\qquad
28C_0+4,
...
\]
\[
1104C_0^2+1657C_0+993,
\qquad
16464C_0^2+36742C_0+19950,
\]
\[
409560C_0^2+955158C_0+417819.
\]
Since $C_0=2^{27}$, these are bounded by the six defining values.
\end{proof}

\begin{lemma}[constant $C_{\ref{lem:abs_deriv_ft_phi3_le},m}$ \auto]\label{constant phi three derivative}
For $m=0$,
\begin{equation}\label{constant phi three derivative bound}
C_{\ref{lem:abs_deriv_ft_phi3_le},0}\le2^2.
\end{equation}
For $m=1$,
\begin{equation}\label{auto:constant-phi-three-derivative-one}
C_{\ref{lem:abs_deriv_ft_phi3_le},1}\le2^{32}.
\end{equation}
For $m=2$,
\begin{equation}\label{auto:constant-phi-three-derivative-two}
C_{\ref{lem:abs_deriv_ft_phi3_le},2}\le2^{61}.
\end{equation}
For $m=3$,
\begin{equation}\label{auto:constant-phi-three-derivative-three}
C_{\ref{lem:abs_deriv_ft_phi3_le},3}\le2^{65}.
\end{equation}
For $m=4$,
\begin{equation}\label{auto:constant-phi-three-derivative-four}
C_{\ref{lem:abs_deriv_ft_phi3_le},4}\le2^{69}.
\end{equation}
For $m=5$,
\begin{equation}\label{auto:constant-phi-three-derivative-five}
C_{\ref{lem:abs_deriv_ft_phi3_le},5}\le2^{73}.
\end{equation}
\end{lemma}
\begin{proof}[Proof \auto]
Substitute $C_0=2^{27}$ in the six defining values.
\end{proof}


\begin{lemma}\label{lem:abs_deriv_ft_Tphi3_le}\using{lem:ft_deriv_mul}\using{lem:abs_deriv_ft_phi3_le}
Let $m\in[N_0)$, $k\in\Z$, and $|\xi|\le1$. Then
...
+C_{\ref{lem:abs_deriv_ft_phi3_le},m+1}.
\]
This is the defining value of $C_{\ref{lem:abs_deriv_ft_Tphi3_le},m}$.
\end{proof}

\begin{lemma}[constant $C_{\ref{lem:abs_deriv_ft_Tphi3_le},m}$ \auto]\label{constant T phi three derivative}\using{lem:abs_deriv_ft_phi3_le}\using{lem:abs_deriv_ft_Tphi3_le}
For $m=0$,
\begin{equation}\label{constant T phi three derivative bound}
C_{\ref{lem:abs_deriv_ft_Tphi3_le},0}\le2^{32}.
\end{equation}
For $m=1$,
\begin{equation}\label{auto:constant-T-phi-three-derivative-one}
C_{\ref{lem:abs_deriv_ft_Tphi3_le},1}\le2^{61}.
\end{equation}
For $m=2$,
\begin{equation}\label{auto:constant-T-phi-three-derivative-two}
C_{\ref{lem:abs_deriv_ft_Tphi3_le},2}\le2^{65}.
\end{equation}
For $m=3$,
\begin{equation}\label{auto:constant-T-phi-three-derivative-three}
C_{\ref{lem:abs_deriv_ft_Tphi3_le},3}\le2^{69}.
\end{equation}
For $m=4$,
\begin{equation}\label{auto:constant-T-phi-three-derivative-four}
C_{\ref{lem:abs_deriv_ft_Tphi3_le},4}\le2^{73}.
\end{equation}
\end{lemma}
\begin{proof}[Proof \auto]
Use the estimates in the proof of Lemma \ref{lem:abs_deriv_ft_phi3_le} in \eqref{auto:T-varphi-three-Fourier-derivative-constant}. For $m=0,1,2,3,4$, respectively, the resulting bounds are smaller than
\[
2^5C_0,
\qquad
97C_0^2,
...
475417C_0^2.
\]
Substitution of $C_0=2^{27}$ gives the five claimed powers of two. For later use, the last estimate also gives
\[
C_{\ref{lem:abs_deriv_ft_Tphi3_le},4}
<\tfrac{117}{128}2^{73},
\]
because $475417<117\cdot4096$.
\end{proof}


\begin{lemma}\label{lem:theta_prim}\using{lem:smoothdecay2}
...
For $m\le N+1$, the product rule gives
\[
...
$\|\widehat\theta^{(m-q)}\|_\infty\le2^{m-q+1}C_0$. For $m=0,1,2,3,4,5$, direct substitution in this finite sum gives upper bounds
\[
2^3C_0,\quad2^6C_0,\quad2^9C_0,\quad2^{13}C_0,\quad2^{17}C_0,\quad2^{21}C_0,
\]
...
\]
For $1\le N<5$, Lemma \ref{lem:smoothdecay2}, the support bound, and the preceding finite estimates give
\[
|\widetilde\theta(u)|+|T\widetilde\theta(u)|
\le2^{5N+6}C_0\langle u\rangle^N.
\]
This proves \eqref{thetaprim_decay} and \eqref{Thetaprim_decay}.
\end{proof}

\begin{lemma}[constant $C_{\ref{lem:theta_prim},N}$ \auto]\label{constant theta primitive}\using{lem:theta_prim}
For $2\le N<N_0$,
\begin{equation}\label{constant theta primitive bound}
C_{\ref{lem:theta_prim},N}\le2^{5N+33}.
\end{equation}
In particular,
\begin{equation}\label{auto:constant-theta-primitive-four}
C_{\ref{lem:theta_prim},4}\le2^{53}.
\end{equation}
\end{lemma}
\begin{proof}[Proof \auto]
Substitute $C_0=2^{27}$ in \eqref{auto:theta-primitive-constant-definition}.
\end{proof}


\begin{lemma}\label{lem:phi4_supp}\using{lem:theta_prim}\using{lem:ft_deriv_mul} We have
...
\begin{lemma}\label{lem:form_pos}\using{M to K}\using{prism BL inequality}\jr{GPT: Refactor this and its proof to use $\Lambda_1$ instead of $\Lambda$ (base the new formulation on what is needed at the places where this is used) \cgpt}
...
\begin{lemma}
\label{lem:Phij_prop}\using{lem:Phipos_v2}
...
|\widehat\psi(\xi)|\le2,
\qquad
|\widehat\psi'(\xi)|\le2^7C_0,
\qquad
|\widehat\psi''(\xi)|\le2^{12}C_0.
\]
...
\begin{lemma}[constant $C_{\ref{lem:Phij_prop}}$ \auto]\label{constant Phij proposition}\using{lem:Phij_prop}
\begin{equation}\label{constant Phij proposition bound}
C_{\ref{lem:Phij_prop}}\le2^{39}.
\end{equation}
\end{lemma}
\begin{proof}[Proof \auto]
This follows from \eqref{auto:Phij-constant-definition} and $C_0=2^{27}$.
\end{proof}
...
\begin{lemma}[$\rho$ kernels - reduction variant] \label{lem:rho-kernels-reduction}\using{mean four scale Gaussian kernel}\using{standard bump properties}\using{four scale Gaussian kernel}  Let $a\in A$. For $h\in\N$ with $h\ge1$ and $j\in\Z$, define
...
\begin{lemma}[constant $C_{\ref{lem:rho-kernels-reduction}}$ \auto]\label{constant rho kernels reduction}\using{constant mean four scale Gaussian kernel}\using{constant four scale Gaussian kernel}\using{mean four scale Gaussian kernel}\using{four scale Gaussian kernel}
\begin{equation}\label{constant rho kernels reduction bound}
C_{\ref{lem:rho-kernels-reduction}}\le2^{65}.
\end{equation}
\end{lemma}
\begin{proof}[Proof \auto]
...
C_{\ref{mean four scale Gaussian kernel},2}
+C_{\ref{four scale Gaussian kernel},2}
\le2^{43}+2^{38}=33\cdot2^{38}.
\]
Multiplication by the defining factor $2^{21}$ gives $33\cdot2^{59}<2^{65}$.
\end{proof}
...
\label{lem:affine-diagonal-cancellation-reduction}\using{W_0 fiber integrals}
...
\label{lem:increase-data-bracket-domination}\using{lem:rho-kernels-reduction}\using{two bump estimate}\using{bump triangle}\using{orthogonal decay}\using{convolution vector}\using{lem:affine-diagonal-cancellation-reduction}\using{Properties of distance of sequences}\using{Operations on spaced sequences}
Let \(a\in A\), and let
$
...
\langle(-w_1-p)\rangle_{(t_0)}^{3/2}\langle(w_1-p)\rangle_{(t_1)}^{3/2}
=
\langle w_1+p\rangle_{(t_0)}^{3/2}\langle w_1-p\rangle_{(t_1)}^{3/2}.$}
\]
Since
...
\begin{equation}
    ...
    \resizebox{0.98\textwidth}{!}{$\displaystyle 2^9\Big( \omega_\lambda(v_0)\omega_{t_1}(v_1) + \omega_\lambda((W_1v)_0) \omega_{t_1}((W_1v)_1) + \omega_{t_0}(v_0)\omega_\lambda(v_1) + \omega_\lambda((W_1v)_0) \omega_{t_0}((W_1v)_1) \Big).$}
\end{equation}
...
\[
\resizebox{0.98\textwidth}{!}{$\displaystyle
\int_{\mathbb R}\omega_{d_\epsilon(j)}(w_0+p)\omega_{t_0(j)}(w_1+p)\omega_{t_1(j)}(w_1-p)\,dp
\leq
2^6\Bigl(\omega_{r_{\epsilon,0}(j)}(v_0)\omega_{t_1(j)}(w_1)
+
\omega_{r_{\epsilon,1}(j)}(v_1)\omega_{t_0(j)}(w_1)\Bigr).$}
\]
Using
...
\begin{equation}
    ...
    \resizebox{0.98\textwidth}{!}{$\displaystyle 2^9\Big( \omega_{r_{\epsilon,0}(j)}(v_0) \omega_{t_1(j)}(v_1) + \omega_{r_{\epsilon,0}(j)}((W_1v)_0) \omega_{t_1(j)}((W_1v)_1) + \omega_{t_0(j)}(v_0) \omega_{r_{\epsilon,1}(j)}(v_1) + \omega_{r_{\epsilon,1}(j)}((W_1v)_0) \omega_{t_0(j)}((W_1v)_1) \Big).$}
\end{equation}
...
\begin{lemma}[constant $C_{\ref{lem:increase-data-bracket-domination},1}$ \auto]\label{constant increase data bracket domination}\using{lem:increase-data-bracket-domination}\using{lem:rho-kernels-reduction}\using{constant rho kernels reduction}
\begin{equation}\label{constant increase data bracket domination bound}
C_{\ref{lem:increase-data-bracket-domination},1}\le2^{75}.
\end{equation}
\end{lemma}
\begin{proof}[Proof \auto]
...
\label{lem:increase-data-Gaussian-expansion}\using{Gaussian domination}\using{lem:increase-data-bracket-domination}\using{Operations on spaced sequences}\using{Properties of distance of sequences}
...
\[
\resizebox{0.98\textwidth}{!}{$\displaystyle \Delta(p_{h,b,m}) = \dist\left( 2^{m_0}\beta_{h,b}^0, 2^{m_1}\beta_{h,b}^1 \right) \leq \dist\left( 2^{m_0}\beta_{h,b}^0, 2^{m_0}\beta_{h,b}^1 \right) \quad+ \dist\left( 2^{m_0}\beta_{h,b}^1, 2^{m_1}\beta_{h,b}^1 \right) = \dist\left( \beta_{h,b}^0, \beta_{h,b}^1 \right) + \dist\left( \beta_{h,b}^1, 2^{m_1-m_0}\beta_{h,b}^1 \right) \leq \Delta(\beta_{h,b}) + |m_1-m_0| \leq \Delta(\beta_{h,b}) + m_0+m_1.$}
\]
...
\[
\resizebox{0.98\textwidth}{!}{$\displaystyle \left\langle (W_{u_{h,b}}v)_0 \right\rangle_{(\beta_{h,b}^0(j))}^{7/6} \left\langle (W_{u_{h,b}}v)_1 \right\rangle_{(\beta_{h,b}^1(j))}^{7/6} \quad\leq C_G^2 2^{7/3} \sum_{m\in\mathbb N^2} 2^{-|m|/6} \qquad\qquad\times \g_{(2^{m_0}\beta_{h,b}^0(j))} \left((W_{u_{h,b}}v)_0\right) \g_{(2^{m_1}\beta_{h,b}^1(j))} \left((W_{u_{h,b}}v)_1\right).$}
\]
...
Therefore,
\[
\resizebox{0.98\textwidth}{!}{$\displaystyle \sum_{m\in\mathbb N^2} 2^{-|m|/6} G_{p_{h,b,m}(j),u_{h,b}}(v) \qquad\leq \tfrac{1}{ \beta_{h,b}^0(j)\beta_{h,b}^1(j) } \sum_{m\in\mathbb N^2} 2^{-7|m|/6} \qquad= \tfrac{1}{ \beta_{h,b}^0(j)\beta_{h,b}^1(j) } \left( \sum_{q\in\mathbb N}2^{-7q/6} \right)^2 \qquad\leq \tfrac{2^2}{ \beta_{h,b}^0(j)\beta_{h,b}^1(j) },$}
\]
...
\begin{lemma}[constant $C_{\ref{lem:increase-data-Gaussian-expansion}}$ \auto]\label{constant increase data Gaussian expansion}\using{lem:increase-data-bracket-domination}\using{Gaussian domination}\using{lem:increase-data-Gaussian-expansion}
\begin{equation}\label{constant increase data Gaussian expansion bound}
C_{\ref{lem:increase-data-Gaussian-expansion}}\le2^{87}.
\end{equation}
\end{lemma}
\begin{proof}[Proof \auto]
The preceding proof gives the sharper bound
\[
C_{\ref{lem:increase-data-bracket-domination},1}
\le33\cdot2^{69}.
\]
Since $C_{\ref{Gaussian domination}}=e^\pi$ and $e^{2\pi}<536$, the defining product satisfies
\[
C_{\ref{lem:increase-data-Gaussian-expansion}}
<2^3\cdot536\cdot33\cdot2^{69}
=17688\cdot2^{72}
<2^{87}.
\]
...
\begin{lemma}
\label{lem:N-reduction}\using{lem:rho-kernels-reduction}\using{convolution vector} Let $a\in A$ and let $\mathcal P$, $(M_j)_{j\in \Z}$, and $(N_{h,j})_{h\in \N,j\in \Z}$,  be as in Proposition~\ref{lem:increase-data-bracket-domination}.
...
\begin{lemma}[constant $C_{\ref{lem:N-reduction}}$ \auto]\label{constant N reduction}\using{lem:N-reduction}\using{lem:rho-kernels-reduction}\using{constant rho kernels reduction}
\begin{equation}\label{constant N reduction bound}
C_{\ref{lem:N-reduction}}\le2^{74}.
\end{equation}
\end{lemma}
\begin{proof}[Proof \auto]
...
\label{P:increase-data-reduction}\using{lem:N-reduction}\using{square root Gaussian difference W0}\using{tensor Wiener}\using{lem:increase-data-Gaussian-expansion}\using{induct positive terms theorem}\using{Monotonicity K}\using{prism BL inequality}\using{lem:increase-data-bracket-domination}
...
\begin{lemma}[constant $C_{\ref{P:increase-data-reduction}}$ \auto]\label{constant increase data reduction}\using{constant induct positive terms theorem}\using{constant increase data Gaussian expansion}\using{lem:increase-data-bracket-domination}\using{P:increase-data-reduction}
\begin{equation}\label{constant increase data reduction bound}
C_{\ref{P:increase-data-reduction}}\le2^{473}.
\end{equation}
\end{lemma}
\begin{proof}[Proof \auto]
Use the sharper estimates from Lemmas \ref{constant induct positive terms theorem} and \ref{constant increase data Gaussian expansion} in the defining product, together with $C_{\ref{lem:increase-data-bracket-domination},0}=2^6$. This gives
\[
C_{\ref{P:increase-data-reduction}}
<\tfrac{127}{128}\tfrac{35}{64}2^{22+358+87+6}
=\tfrac{4445}{8192}2^{473}
<2^{473}.
\]
\end{proof}
...
\label{P:diagonal-band-reduction}\using{P:increase-data-reduction}\using{convolution vector}\using{Cauchy-Schwarz at k}\using{lem:N-reduction}\using{Cauchy-Schwarz at n-1}
...
\begin{lemma}[constant $C_{\ref{P:diagonal-band-reduction}}$ \auto]\label{constant diagonal band reduction}\using{constant N reduction}\using{constant increase data reduction}
\begin{equation}\label{constant diagonal band reduction bound}
C_{\ref{P:diagonal-band-reduction}}\le2^{476}.
\end{equation}
\end{lemma}
\begin{proof}[Proof \auto]
Lemmas \ref{constant N reduction} and \ref{constant increase data reduction} bound the first term in the defining sum by
\[
2^4(2^{74}2^{473})^{1/2}<2^{278}.
\]
The sharper estimate in the proof of Lemma \ref{constant increase data reduction} bounds the second term by
\[
\tfrac{4445}{8192}2^{476}.
\]
Since $2^{278}<\frac{35}{8192}2^{476}$, their sum is smaller than
\[
\tfrac{4480}{8192}2^{476}=\tfrac{35}{64}2^{476}<2^{476}.
\]
\end{proof}


\begin{lemma}\label{lem:L1-reduction}\using{lem:affine-diagonal-cancellation-reduction}
...
\label{P:vanishing-diagonal-reduction}\using{P:diagonal-band-reduction}\using{prism BL inequality}\using{M to K}\using{lem:L1-reduction}
...
\begin{proposition}\label{P:one-scale-estimate-window}\using{prism BL inequality}\using{M to K}\using{lem:smoothdecay2}
...
\end{equation}
where $C_{\ref{P:one-scale-estimate-window}}=2^9C_0^2$.
...
|\phi_0(x)|
\le 2C_{\ref{lem:smoothdecay2},2}C_0\langle x\rangle^2
=8C_0\langle x\rangle^2.
\end{equation}
Since $\int_\R\langle x\rangle^2\,dx=2$, we have $\|\phi_0\|_1\le16C_0$. Combining this with \eqref{E:telescoping} and \eqref{E:est-l1-norm} yields \eqref{E:telescoping-estimate}.
\end{proof}

\begin{lemma}[constant $C_{\ref{P:one-scale-estimate-window}}$ \auto]\label{auto:constant-one-scale-window}
\begin{equation}\label{auto:constant-one-scale-window-bound}
C_{\ref{P:one-scale-estimate-window}}\le2^{63}.
\end{equation}
\end{lemma}
\begin{proof}[Proof \auto]
Substitute $C_0=2^{27}$ in the defining value $2^9C_0^2$.
\end{proof}
...
\label{P:induct-positive-terms-reduction-non-whitney}\using{P:one-scale-estimate-window}\using{L:fourier-transform-window}\using{Operations on spaced sequences}\using{lem:smoothdecay2}\using{lem:scaleest}\using{P:vanishing-diagonal-reduction}\using{P:diagonal-band-reduction}
Let $a\in A$ and let
...
where $C_{\ref{P:induct-positive-terms-reduction-non-whitney}}=C_{\ref{P:one-scale-estimate-window}}+ 64 \max(1,(C_0/2\pi)^4) C_{\ref{lem:smoothdecay2},2}^2C_{\ref{P:diagonal-band-reduction}}$.
\end{proposition}
...
\[
|\phi_i(x)| \leq 2 C_{\ref{lem:smoothdecay2},2} \max(1,(C_0/2\pi)^2) \langle x \rangle^2.
\]
Thus, for $v\in \R^2$,
\[
|M_{1,j}(v)|
\leq 4 C_{\ref{lem:smoothdecay2},2}^2 \max(1,(C_0/2\pi)^4)\Big[
...
\[
64 C_{\ref{lem:smoothdecay2},2}^2 \max(1,(C_0/2\pi)^4)
...
\[
\|\mathbf{M}_1\|_{\rm M(1)} \leq 64 \max(1,(C_0/2\pi)^4) C_{\ref{lem:smoothdecay2},2}^2C_{\ref{P:diagonal-band-reduction}}.
\]
\end{proof}

\begin{lemma}[constant $C_{\ref{P:induct-positive-terms-reduction-non-whitney}}$ \auto]\label{constant non Whitney reduction}\using{auto:constant-one-scale-window}\using{lem:smoothdecay2}\using{P:diagonal-band-reduction}\using{constant diagonal band reduction}\using{P:induct-positive-terms-reduction-non-whitney}
\begin{equation}\label{constant non Whitney reduction bound}
C_{\ref{P:induct-positive-terms-reduction-non-whitney}}\le2^{583}.
\end{equation}
\end{lemma}
\begin{proof}[Proof \auto]
Use Lemma \ref{auto:constant-one-scale-window}, $C_0=2^{27}$, $C_{\ref{lem:smoothdecay2},2}=2^2$, and the sharper estimate
\[
C_{\ref{P:diagonal-band-reduction}}<\tfrac{35}{64}2^{476}
\]
from the proof of Lemma \ref{constant diagonal band reduction}. Since $\pi>3$, the second term in the defining sum is smaller than
\[
2^{10}\left(\tfrac{2^{27}}{2\pi}\right)^4\tfrac{35}{64}2^{476}
<\tfrac{70}{81}2^{583}.
\]
The first term is $2^{63}<\frac1{81}2^{583}$. Hence
\[
C_{\ref{P:induct-positive-terms-reduction-non-whitney}}
<\tfrac{71}{81}2^{583}<2^{583}.
\]
\end{proof}


\begin{proposition}[induct positive terms - reduction variant, non-Whitney, skip terms]
\label{P:induct-positive-terms-reduction-non-whitney-skip}\using{positive terms}\using{P:induct-positive-terms-reduction-non-whitney}
Let $a\in A$ and let
...
\end{equation}
Let $(M_j)_{j\in \Z}$ be as in Proposition~\ref{P:induct-positive-terms-reduction-non-whitney} and let $J\in\N$ with $J\ge1$. By~\eqref{E:intermediate-terms-nonnegative} and Proposition~\ref{P:induct-positive-terms-reduction-non-whitney}, we get that for every $\mathbf{F} \in \mathfrak{F}$,
\[
...
\begin{lemma}[constant $C_{\ref{P:induct-positive-terms-reduction-non-whitney-skip}}$ \auto]\label{constant non Whitney skip reduction}\using{constant non Whitney reduction}\using{P:induct-positive-terms-reduction-non-whitney-skip}
\begin{equation}\label{constant non Whitney skip reduction bound}
C_{\ref{P:induct-positive-terms-reduction-non-whitney-skip}}\le2^{584}.
\end{equation}
\end{lemma}
\begin{proof}[Proof \auto]
The factor $2^{1-2^{2-n}}$ is at most $2$. The sharper estimate in the proof of Lemma \ref{constant non Whitney reduction} therefore gives
\[
C_{\ref{P:induct-positive-terms-reduction-non-whitney-skip}}
<\tfrac{71}{81}2^{584}<2^{584}.
\]
\end{proof}


\begin{proposition}[induct positive terms - reduction variant, Whitney, with gap]
\label{P:induct-positive-terms-reduction-whitney-gap}\using{lem:Phij_prop}\using{lem:form_pos}\using{P:induct-positive-terms-reduction-non-whitney-skip}\using{P:vanishing-diagonal-reduction}\using{lem:smoothdecay2}\using{lem:scaleest}\using{P:diagonal-band-reduction}
...
+2^{19}(C_0^2+C_{\ref{lem:Phij_prop}}^2)C_{\ref{P:diagonal-band-reduction}}.
\end{equation}
\end{proposition}
...
\[
|\phi_i(x)|\le2^3C_0\langle x\rangle^2
\]
for $i\in[2)$. Thus, by Lemma \ref{lem:scaleest},
\[
|(\phi_{0,2^{-5}}-\phi_{1,2^5})^{\otimes2}(v)|
\le2^{18}C_0^2\langle v_0\rangle^2\langle v_1\rangle^2.
\]
...
where
\[
c=2^{19}(C_0^2+C_{\ref{lem:Phij_prop}}^2).
\]
Proposition \ref{P:vanishing-diagonal-reduction} now yields
\[
\|(M_{3,j})_{j\in\Z}\|_{\rm M(1)}
\le2^{19}(C_0^2+C_{\ref{lem:Phij_prop}}^2)
C_{\ref{P:diagonal-band-reduction}},
\]
which proves the claim.
\end{proof}

\begin{lemma}[constant $C_{\ref{P:induct-positive-terms-reduction-whitney-gap}}$ \auto]\label{constant Whitney gap reduction}\using{constant non Whitney skip reduction}\using{P:induct-positive-terms-reduction-whitney-gap}\using{constant Phij proposition}\using{constant diagonal band reduction}
\begin{equation}\label{constant Whitney gap reduction bound}
C_{\ref{P:induct-positive-terms-reduction-whitney-gap}}\le2^{585}.
\end{equation}
\end{lemma}
\begin{proof}[Proof \auto]
The sharper estimate in the proof of Lemma \ref{constant non Whitney skip reduction} bounds the first term in \eqref{auto:Whitney-gap-constant-definition} by
\[
\tfrac{71}{81}2^{585}.
\]
Lemmas \ref{constant Phij proposition} and \ref{constant diagonal band reduction}, together with $C_0=2^{27}$, bound the second term by
\[
2^{19}(2^{54}+2^{78})2^{476}<2^{574}<\tfrac1{81}2^{585}.
\]
Thus
\[
C_{\ref{P:induct-positive-terms-reduction-whitney-gap}}
<\tfrac{72}{81}2^{585}=\tfrac89 2^{585}<2^{585}.
\]
\end{proof}


\pd{\begin{equation}\label{auto:smooth-decay-estimate-restatement}|\phi(x)|\leq 2^{N} \max(\|\widehat{\phi}\|_\infty, (2\pi )^{-N}\|\widehat{\phi}^{(N)}\|_\infty)|\supp (\widehat{\phi})|   \langle x\rangle^{N}
 \end{equation}
 with $C_{\ref{lem:smoothdecay2}, N}=2^{N} $. }


\begin{proposition}[induct positive terms - reduction variant, Whitney]
\label{P:induct-positive-terms-reduction-whitney}\using{P:induct-positive-terms-reduction-whitney-gap}
...
\begin{lemma}[constant $C_{\ref{P:induct-positive-terms-reduction-whitney}}$ \auto]\label{constant Whitney reduction}\using{constant Whitney gap reduction}\using{P:induct-positive-terms-reduction-whitney}
\begin{equation}\label{constant Whitney reduction bound}
C_{\ref{P:induct-positive-terms-reduction-whitney}}\le2^{589}.
\end{equation}
\end{lemma}
\begin{proof}[Proof \auto]
Using the sharper estimate in the proof of Lemma \ref{constant Whitney gap reduction},
\[
C_{\ref{P:induct-positive-terms-reduction-whitney}}
<11\cdot\tfrac89 2^{585}
=\tfrac{11}{18}2^{589}<2^{589}.
\]
\end{proof}


\begin{proposition}[induct positive terms - reduction variant, Whitney, product-type]
\label{P:induct-positive-terms-reduction-whitney-product}\using{P:induct-positive-terms-reduction-whitney-gap}\using{P:induct-positive-terms-reduction-whitney}
Let $a\in A$ and let
...
|\psi(x)|\le\langle x\rangle^4
\end{equation}
for every $x\in\R$. Then, with $\M=(M_j)_{j\in\Z}$,
...
=72C_{\ref{P:induct-positive-terms-reduction-whitney}}.
\end{equation}
\end{proposition}

\begin{proof}[Proof \auto]
Fourier differentiation and the decay assumption give
\[
\|\widehat\psi\|_\infty\le1,
\qquad
\|\widehat\psi'\|_\infty\le3,
\qquad
\|\widehat\psi''\|_\infty\le27.
\]
Set $\Psi=72^{-1}\psi\otimes\psi$. Then $\Psi$ is symmetric and satisfies \eqref{p1-on-W_pos} and \eqref{p1-on-W_supp}. The zeroth, first, and second derivatives of $\widehat\psi(\xi)\widehat\psi(-\xi)$ are bounded by $1$, $6$, and $72$, respectively, so \eqref{p1-on-W_der} holds. Moreover,
\[
|\Psi(v)|
\le72^{-1}\langle v_0\rangle^4\langle v_1\rangle^4
\le\langle v_0\rangle^{3/2}\langle v_1\rangle^{3/2},
\]
so \eqref{p1-on-W_decay} holds. Proposition \ref{P:induct-positive-terms-reduction-whitney} applied to $72^{-1}M_j$ proves the claim.
\end{proof}

\begin{lemma}[constant $C_{\ref{P:induct-positive-terms-reduction-whitney-product}}$ \auto]\label{constant Whitney product reduction}\using{constant Whitney reduction}\using{P:induct-positive-terms-reduction-whitney-product}
\begin{equation}\label{constant Whitney product reduction bound}
C_{\ref{P:induct-positive-terms-reduction-whitney-product}}\le2^{595}.
\end{equation}
\end{lemma}
\begin{proof}[Proof \auto]
The sharper estimate in the proof of Lemma \ref{constant Whitney reduction} gives
\[
C_{\ref{P:induct-positive-terms-reduction-whitney-product}}
<72\cdot\tfrac{11}{18}2^{589}
=\tfrac{11}{16}2^{595}<2^{595}.
\]
\end{proof}
...
\begin{lemma}\label{lem:main_aux1}\using{A to Lambda}\using{P:induct-positive-terms-reduction-whitney-product}
...
\end{equation}
\begin{equation}\label{main_aux1_decay}
|\psi(u)|\le\langle u\rangle^4.
\end{equation}
Then for every $t\in[1,2]$, $J\ge1$, and every increasing sequence of integers $(k_j)_{j\in[J)}$,
...
=2^6C_{\ref{P:induct-positive-terms-reduction-whitney-product}}.
\end{equation}
\end{lemma}

\begin{proof}[Proof \auto]
By Lemma \ref{A to Lambda} and linearity, the left-hand side equals $\Lambda_1(M_t)(\F)$, where
\[
M_t=\sum_{j\in[J)}(\psi_{(2^{k_j}t)})^{\otimes2}.
\]
Apply Proposition \ref{P:induct-positive-terms-reduction-whitney-product} with $2^{-3}\psi_{(t)}$ in place of $\psi$, and multiply the resulting estimate by $2^6$. The Fourier support condition follows from \eqref{main_aux1_supp} and $t\in[1,2]$. Moreover,
\[
2^{-3}|\psi_{(t)}(u)|
\le2^{-3}\langle u\rangle_{(t)}^4
\le\langle u\rangle^4,
\]
so \eqref{Whitney product decay assumption} holds.
\end{proof}

\begin{lemma}[constant $C_{\ref{lem:main_aux1}}$ \auto]\label{constant main auxiliary one}\using{constant Whitney product reduction}\using{lem:main_aux1}
\begin{equation}\label{constant main auxiliary one bound}
C_{\ref{lem:main_aux1}}\le2^{601}.
\end{equation}
\end{lemma}
\begin{proof}[Proof \auto]
The sharper estimate in the proof of Lemma \ref{constant Whitney product reduction} and \eqref{auto:main-auxiliary-one-constant-definition} give
\[
C_{\ref{lem:main_aux1}}<\tfrac{11}{16}2^{601}<2^{601}.
\]
\end{proof}

\begin{lemma}[Long and short variation]\label{lem:shortlongftc_reduction}\using{lem:shortlongjumps}\using{lem:ftccs-R}\using{lem:ftc_ATphi}\using{lem:main_aux1}
...
Thus for every $x\in\mathbb{R}^n$ and $k\in\mathbb{Z}$,
\[ \|A_t(\phi)(x)\|_{V_{2,J}(t\in [2^k, 2^{k+1}])}^2 \le\int_{1}^2 |A_{2^k t}(T\phi)(x)|^2\,\tfrac{dt}{t}.  \]
Integrating over $x\in\mathbb{R}^n$ and interchanging the integrals over $x$ and $t$ gives
...
\begin{lemma}\label{lem:mainbump1_long1}\using{lem:main_aux1}\using{lem:smoothdecay2}
...
=(4C_0C_{\ref{lem:smoothdecay2},4})^2C_{\ref{lem:main_aux1}}.
\end{equation}
\end{lemma}
...
\[
c_1=4C_0C_{\ref{lem:smoothdecay2},4}.
\]
The Fourier transform of $\phi_0-\phi_1$ is supported in $\mathrm{Ann}_1(1,2)$. By \eqref{windowbound},
\[
\|\widehat{\phi_0}^{(4)}-\widehat{\phi_1}^{(4)}\|_\infty\le2C_0.
\]
Lemma \ref{lem:smoothdecay2}, applied with $N=4$, therefore gives
\[
|\phi_0(u)-\phi_1(u)|
\le4C_0C_{\ref{lem:smoothdecay2},4}\langle u\rangle^4.
\]
Thus \eqref{main_aux1_supp} and \eqref{main_aux1_decay} hold, and the result follows.
\end{proof}

\begin{lemma}[constant $C_{\ref{lem:mainbump1_long1}}$ \auto]\label{constant main bump one long one}\using{lem:smoothdecay2}\using{constant main auxiliary one}\using{lem:mainbump1_long1}
\begin{equation}\label{constant main bump one long one bound}
C_{\ref{lem:mainbump1_long1}}\le2^{667}.
\end{equation}
\end{lemma}
\begin{proof}[Proof \auto]
Use $C_0=2^{27}$, $C_{\ref{lem:smoothdecay2},4}=2^4$, and the sharper estimate from Lemma \ref{constant main auxiliary one} in \eqref{auto:main-bump-one-long-one-constant-definition}. This gives
\[
C_{\ref{lem:mainbump1_long1}}<\tfrac{11}{16}2^{667}<2^{667}.
\]
\end{proof}


\begin{lemma}\label{lem:mainbump1_long2}\using{Extension of sequences}\using{A to Lambda}\using{P:induct-positive-terms-reduction-non-whitney-skip}\using{Operations on spaced sequences}
...
\begin{lemma}[constant $C_{\ref{lem:mainbump1_long2}}$ \auto]\label{constant main bump one long two}\using{P:induct-positive-terms-reduction-non-whitney-skip}\using{constant non Whitney skip reduction}\using{lem:mainbump1_long2}
\begin{equation}\label{constant main bump one long two bound}
C_{\ref{lem:mainbump1_long2}}\le2^{585}.
\end{equation}
\end{lemma}
\begin{proof}[Proof \auto]
The defining value is twice $C_{\ref{P:induct-positive-terms-reduction-non-whitney-skip}}$. The sharper estimate in the proof of Lemma \ref{constant non Whitney skip reduction} gives
\[
C_{\ref{lem:mainbump1_long2}}<\tfrac{71}{81}2^{585}<2^{585}.
\]
\end{proof}


\begin{lemma}\label{lem:mainbump1_long}\using{lem:mainbump1_long2}\using{lem:mainbump1_long1}
Let $J\ge 1$. Then
...
\begin{lemma}[constant $C_{\ref{lem:mainbump1_long}}$ \auto]\label{constant main bump one long}\using{constant main bump one long one}\using{constant main bump one long two}\using{lem:mainbump1_long}
\begin{equation}\label{constant main bump one long bound}
C_{\ref{lem:mainbump1_long}}\le2^{668}.
\end{equation}
\end{lemma}
\begin{proof}[Proof \auto]
Use Lemmas \ref{constant main bump one long one} and \ref{constant main bump one long two} in the defining sum. More precisely, their sharper estimates give
\[
C_{\ref{lem:mainbump1_long}}
<2\left(\tfrac{11}{16}2^{667}+2^{585}\right)
<\tfrac34 2^{668}<2^{668}.
\]
\end{proof}


\begin{lemma}\label{lem:mainbump1}\using{lem:smoothdecay2}\using{lem:shortlongftc_reduction}\using{lem:mainbump1_long}\using{lem:ft_deriv_mul}
Let $J\ge1$. Then
...
=2^{12}C_0^2C_{\ref{lem:smoothdecay2},4}^2C_{\ref{lem:main_aux1}}
...
Let
\[
c_1=2^4C_0C_{\ref{lem:smoothdecay2},4}.
\]
...
\]
The long variation assumption follows from Lemma \ref{lem:mainbump1_long}. It remains to verify the assumptions on $c_1^{-1}T\phi_0$.

By Lemma \ref{lem:ft_deriv_mul}, $\widehat{T\phi_0}$ is supported in $\mathrm{Ann}_1(1,2)$ and
\[
\|(\widehat{T\phi_0})^{(4)}\|_\infty
\le4\|\widehat{\phi_0}^{(4)}\|_\infty
+\|\widehat{\phi_0}^{(5)}\|_\infty
\le5C_0.
\]
Lemma \ref{lem:smoothdecay2} with $N=4$ gives
\[
|T\phi_0(u)|
\le10C_0C_{\ref{lem:smoothdecay2},4}\langle u\rangle^4
\le c_1\langle u\rangle^4.
\]
Lemma \ref{lem:shortlongftc_reduction} now gives the stated constant.
\end{proof}

\begin{lemma}[constant $C_{\ref{lem:mainbump1}}$ \auto]\label{constant main bump one}\using{lem:smoothdecay2}\using{constant main auxiliary one}\using{lem:mainbump1}\using{constant main bump one long}
\begin{equation}\label{constant main bump one bound}
C_{\ref{lem:mainbump1}}\le2^{675}.
\end{equation}
\end{lemma}
\begin{proof}[Proof \auto]
Use $C_0=2^{27}$, $C_{\ref{lem:smoothdecay2},4}=2^4$, and the sharper estimate from Lemma \ref{constant main auxiliary one} in the first term of \eqref{auto:main-bump-one-constant-definition}. It is smaller than
\[
\tfrac{11}{16}2^{675}.
\]
Lemma \ref{constant main bump one long} bounds the second term by $2^{669}$. Hence the defining sum is smaller than
\[
\left(\tfrac{11}{16}+\tfrac1{64}\right)2^{675}<2^{675}.
\]
\end{proof}


\begin{lemma}\label{lem:main_aux2}\using{lem:shortlongftc_reduction}\using{lem:main_aux1}\using{lem:bootstrap}
...
|T\psi(u)|\le\langle u\rangle^4
\end{equation}
for all $u\in \R$.
Then
\begin{equation}\label{main_aux2}
\|A_{t}(\psi)\|_{V_{2,J}(t\in(0,\infty);L^2)}^2 \le C_{\ref{lem:main_aux2}} J^{\alpha(n)},
\end{equation}
where $C_{\ref{lem:main_aux2}} = 24 C_{\ref{lem:main_aux1}}$.
\end{lemma}
...
We will show that \eqref{shortlongftc_reduction1} holds with $A=4C_{\ref{lem:main_aux1}}J^{\alpha(n)}$, i.e.
...
\begin{lemma}[constant $C_{\ref{lem:main_aux2}}$ \auto]\label{constant main auxiliary two}\using{constant main auxiliary one}\using{lem:main_aux2}
\begin{equation}\label{constant main auxiliary two bound}
C_{\ref{lem:main_aux2}}\le2^{606}.
\end{equation}
\end{lemma}
\begin{proof}[Proof \auto]
The sharper estimate from Lemma \ref{constant main auxiliary one} gives
\[
C_{\ref{lem:main_aux2}}
<24\cdot\tfrac{11}{16}2^{601}
=\tfrac{33}{64}2^{606}<2^{606}.
\]
\end{proof}


\begin{lemma}\label{lem:mainbump2}\using{lem:rescaling}\using{lem:main_aux2}\using{lem:abs_deriv_ft_Tphi3_le}\using{lem:smoothdecay2}\using{lem:ft_phi3_eq}\using{lem:abs_deriv_ft_phi3_le}\using{lem:ft_deriv_mul}
For all $k\ge-2$,
...
=2^2C_{\ref{lem:main_aux2}}
C_{\ref{lem:abs_deriv_ft_Tphi3_le},4}^2
C_{\ref{lem:smoothdecay2},4}^2.
\end{equation}
\end{lemma}
...
\[
c_1=2C_{\ref{lem:abs_deriv_ft_Tphi3_le},4}C_{\ref{lem:smoothdecay2},4}.
\]
By Lemma \ref{lem:ft_phi3_eq},
...
\end{equation}
Lemma \ref{lem:abs_deriv_ft_phi3_le} with $m=4$, followed by Lemma \ref{lem:smoothdecay2} with $N=4$, gives
\begin{equation}\label{mainbump2_dec1}
|\varphi_{3,k}(u)|
\le2C_{\ref{lem:abs_deriv_ft_Tphi3_le},4}C_{\ref{lem:smoothdecay2},4}\langle u\rangle^4.
\end{equation}
Similarly, Lemma \ref{lem:abs_deriv_ft_Tphi3_le} with $m=4$ gives
\begin{equation}\label{mainbump2_dec2}
|T\varphi_{3,k}(u)|
\le2C_{\ref{lem:abs_deriv_ft_Tphi3_le},4}C_{\ref{lem:smoothdecay2},4}\langle u\rangle^4.
\end{equation}
The support of $\widehat{T\varphi_{3,k}}$ is contained in the same annulus by Lemma \ref{lem:ft_deriv_mul}. Thus all assumptions of Lemma \ref{lem:main_aux2} hold.
\end{proof}

\begin{lemma}[constant $C_{\ref{lem:mainbump2}}$ \auto]\label{constant main bump two}\using{constant main auxiliary two}\using{lem:abs_deriv_ft_Tphi3_le}\using{constant T phi three derivative}\using{lem:smoothdecay2}\using{lem:mainbump2}
\begin{equation}\label{constant main bump two bound}
C_{\ref{lem:mainbump2}}\le2^{761}.
\end{equation}
\end{lemma}
\begin{proof}[Proof \auto]
Use the sharper estimate from Lemma \ref{constant main auxiliary two}, the estimate
\[
C_{\ref{lem:abs_deriv_ft_Tphi3_le},4}<\tfrac{117}{128}2^{73}
\]
from Lemma \ref{constant T phi three derivative}, and $C_{\ref{lem:smoothdecay2},4}=2^4$ in \eqref{auto:main-bump-two-constant-definition}. This gives
\[
C_{\ref{lem:mainbump2}}
<\tfrac{33}{64}\left(\tfrac{117}{128}\right)^2 2^{762}
<\tfrac{7}{8}2^{761}<2^{761}.
\]
\end{proof}


\begin{lemma}\label{lem:leftbump}\using{lem:rescaling}\using{lem:main_aux2}\using{lem:theta_prim}
For every $k\le-1$,
...
C_{\ref{lem:leftbump}}
=C_{\ref{lem:main_aux2}}C_{\ref{lem:theta_prim},4}^2.
\end{equation}
\end{lemma}
...
\]
By Lemma \ref{lem:rescaling}, it suffices to apply Lemma \ref{lem:main_aux2} to $C_{\ref{lem:theta_prim},4}^{-1}\widetilde\theta$. The support and decay assumptions for this function and its image under $T$ are precisely \eqref{thetaprim_supp}, \eqref{Tthetaprim_supp}, \eqref{thetaprim_decay}, and \eqref{Thetaprim_decay} with $N=4$.
\end{proof}

\begin{lemma}[constant $C_{\ref{lem:leftbump}}$ \auto]\label{constant left bump}\using{constant main auxiliary two}\using{constant theta primitive}\using{lem:leftbump}
\begin{equation}\label{constant left bump bound}
C_{\ref{lem:leftbump}}\le2^{712}.
\end{equation}
\end{lemma}
\begin{proof}[Proof \auto]
Use Lemmas \ref{constant main auxiliary two} and \ref{constant theta primitive}, with $N=4$, in \eqref{auto:left-bump-constant-definition}. Their sharper estimates give
\[
C_{\ref{lem:leftbump}}<\tfrac{33}{64}2^{606+106}<2^{712}.
\]
\end{proof}


\begin{lemma}\label{lem:leftbump1_short1}\using{A to Lambda}\using{lem:theta_prim}\using{lem:thetat_offcenter}\using{P:induct-positive-terms-reduction-whitney}\using{lem:int_fct}\using{lem:phi4_supp}\using{P:induct-positive-terms-reduction-whitney-gap}
...
C_{\ref{lem:theta_prim},4}^2C_{\ref{lem:thetat_offcenter}}.
\end{equation}
\end{lemma}

\begin{proof}[Proof \auto]
By Lemma \ref{A to Lambda} and linearity, the left-hand side equals $\Lambda_1(M)(\F)$, where
\[
M=\sum_{j\in[J)}\Psi_{(2^{k_j})},
\qquad
\Psi=2^{-(1+\gamma)k}\int_1^2(\varphi_{4,k})_{(t)}^{\otimes2}\,\tfrac{dt}{t}.
\]
Put
\[
c_1=2^6C_{\ref{lem:theta_prim},4}^2C_{\ref{lem:thetat_offcenter}}.
\]
...
\[
|(\varphi_{4,k})_{(t)}(u)|
\le2^{k+3}C_{\ref{lem:theta_prim},4}
\langle u-t2^{-k}\rangle^4.
\]
Lemma \ref{lem:thetat_offcenter} therefore gives
\[
|\Psi(u,v)|
\le2^6C_{\ref{lem:theta_prim},4}^2C_{\ref{lem:thetat_offcenter}}
2^{(2-\gamma)k}
\langle2^k(u+v)\rangle^{3/2}
\langle u-v\rangle^{3/2}.
\]
Since $2-\gamma=3/2$ and $2^k\le1$,
\[
2^{(2-\gamma)k}\langle2^k(u+v)\rangle^{3/2}
\le\langle u+v\rangle^{3/2}.
\]
This proves \eqref{p1-on-W_decay} for $c_1^{-1}\Psi$.

For $m\in[3)$, Fourier differentiation gives
\[
\left|\tfrac{d^m}{d\xi^m}\widehat\Psi(\xi,-\xi)\right|
\le(2\pi)^m\int_{\R^2}|u-v|^m|\Psi(u,v)|\,d(u,v).
\]
In the defining integral for $\Psi$, make the changes of variables
\[
u=t(x+2^{-k}),
\qquad
v=t(y+2^{-k}).
...
Since
\[
\int_\R |x|^m\langle x\rangle^4\,dx\le1
\]
for $m\in[3)$, the last integral is at most
\[
2^9C_{\ref{lem:theta_prim},4}^2 2^{(1-\gamma)k}
\le c_1.
\]
Thus \eqref{p1-on-W_der} holds. Proposition \ref{P:induct-positive-terms-reduction-whitney} proves the result.
\end{proof}

\begin{lemma}[constant $C_{\ref{lem:leftbump1_short1}}$ \auto]\label{constant left bump one short one}\using{constant Whitney reduction}\using{constant theta primitive}\using{lem:thetat_offcenter}\using{lem:leftbump1_short1}
\begin{equation}\label{constant left bump one short one bound}
C_{\ref{lem:leftbump1_short1}}\le2^{706}.
\end{equation}
\end{lemma}
\begin{proof}[Proof \auto]
Use the sharper estimate from Lemma \ref{constant Whitney reduction}, Lemma \ref{constant theta primitive} with $N=4$, and $C_{\ref{lem:thetat_offcenter}}=2^5$ in \eqref{auto:left-bump-one-short-one-constant-definition}. This gives
\[
C_{\ref{lem:leftbump1_short1}}<\tfrac{11}{18}2^{706}<2^{706}.
\]
\end{proof}


\begin{lemma}\label{lem:leftbump1_short2}\using{A to Lambda}\using{lem:theta_prim}\using{lem:theta_decay}\using{lem:thetat_offcenter}\using{P:induct-positive-terms-reduction-whitney}\using{lem:int_fct}\using{lem:phi4_supp}\using{lem:leftbump1_short1}\using{P:induct-positive-terms-reduction-whitney-gap}
Let $\gamma=\frac12$. For every $k\le-1$ and every increasing sequence of integers $(k_j)_{j\in[J)}$,
...
=2^{10}C_{\ref{P:induct-positive-terms-reduction-whitney}}
C_{\ref{lem:thetat_offcenter}}
\max\bigl(C_{\ref{lem:theta_prim},4},C_{\ref{lem:theta_decay},4},C_{\ref{lem:theta_decay},5}\bigr)^2.
\end{equation}
\end{lemma}
...
Put
\[
C=\max\bigl(C_{\ref{lem:theta_prim},4},C_{\ref{lem:theta_decay},4},C_{\ref{lem:theta_decay},5}\bigr),
\qquad
c_1=2^{10}C^2C_{\ref{lem:thetat_offcenter}}.
\]
We apply Proposition \ref{P:induct-positive-terms-reduction-whitney} to $c_1^{-1}\Psi$. Symmetry, positivity, and the Fourier support condition follow from Lemmas \ref{lem:int_fct} and \ref{lem:phi4_supp}.
...
+\theta(u-2^{-k}).
\]
Lemmas \ref{lem:theta_prim} and \ref{lem:theta_decay} give, for $t\in[1,2]$,
\[
|(T\varphi_{4,k})_{(t)}(u)|
\le2^5C\langle u-t2^{-k}\rangle^4.
\]
Lemma \ref{lem:thetat_offcenter} now gives
\[
|\Psi(u,v)|
\le2^{10}C^2C_{\ref{lem:thetat_offcenter}}
2^{(2-\gamma)k}
\langle2^k(u+v)\rangle^{3/2}
\langle u-v\rangle^{3/2}.
\]
As in the proof of Lemma \ref{lem:leftbump1_short1}, this proves \eqref{p1-on-W_decay} for $c_1^{-1}\Psi$.

For $m\in[3)$, the same changes of variables as in that proof and
\[
|T\varphi_{4,k}(2^{-k}+x)|\le3C\langle x\rangle^4
\]
give
\[
(2\pi)^m\int_{\R^2}|u-v|^m|\Psi(u,v)|\,d(u,v)
\le2^{13}C^2 2^{(1-\gamma)k}
\le c_1.
\]
Thus \eqref{p1-on-W_der} holds, and Proposition \ref{P:induct-positive-terms-reduction-whitney} completes the proof.
\end{proof}

\begin{lemma}[constant $C_{\ref{lem:leftbump1_short2}}$ \auto]\label{constant left bump one short two}\using{constant theta primitive}\using{constant theta decay}\using{lem:theta_prim}\using{lem:theta_decay}\using{constant Whitney reduction}\using{lem:thetat_offcenter}\using{lem:leftbump1_short2}
\begin{equation}\label{constant left bump one short two bound}
C_{\ref{lem:leftbump1_short2}}\le2^{710}.
\end{equation}
\end{lemma}
\begin{proof}[Proof \auto]
Lemmas \ref{constant theta primitive} and \ref{constant theta decay} give
\[
\max\bigl(C_{\ref{lem:theta_prim},4},C_{\ref{lem:theta_decay},4},C_{\ref{lem:theta_decay},5}\bigr)
\le2^{53}.
\]
Use this, the sharper estimate from Lemma \ref{constant Whitney reduction}, and $C_{\ref{lem:thetat_offcenter}}=2^5$ in \eqref{auto:left-bump-one-short-two-constant-definition}. The result is smaller than
\[
\tfrac{11}{18}2^{710}<2^{710}.
\]
\end{proof}


\begin{lemma}\label{lem:leftbump1_long}\using{lem:bootstrap}\using{A to Lambda}\using{lem:theta_prim}\using{P:induct-positive-terms-reduction-whitney}\using{lem:phi4_supp}\using{lem:form_pos}\using{lem:thetat_offcenter}\using{P:induct-positive-terms-reduction-whitney-gap}
...
=2^{10}C_{\ref{P:induct-positive-terms-reduction-whitney}}C_{\ref{lem:theta_prim},4}^2.
\end{equation}
\end{lemma}
...
\]
By Lemma \ref{A to Lambda}, the sum is $\Lambda_1(M)(\F)$ with
\[
M=\sum_{j\in[J)}(\varphi_{4,k})_{(2^{k_j})}^{\otimes2}.
\]
Set
\[
\Psi=2^{-\gamma k}\varphi_{4,k}^{\otimes2},
\qquad
c_1=2^8C_{\ref{lem:theta_prim},4}^2.
\]
...
Lemma \ref{lem:theta_prim} and the special-coordinate estimate used in the proof of Lemma \ref{lem:thetat_offcenter} give
\[
|\Psi(u,v)|
\le C_{\ref{lem:theta_prim},4}^2
2^{(2-\gamma)k}
\langle u+v-2^{1-k}\rangle^2
\langle u-v\rangle^2.
\]
For $k\le-1$ and $s\in\R$,
\[
2^{(2-\gamma)k}\langle s-2^{1-k}\rangle^2
\le2^4\langle s\rangle^{3/2}.
\]
Indeed, this follows by considering $|s|\le2^{-k}$, $2^{-k}<|s|\le2^{2-k}$, and $|s|>2^{2-k}$. Hence \eqref{p1-on-W_decay} holds for $c_1^{-1}\Psi$.

For $m\in[3)$, the change of variables $u=x+2^{-k}$, $v=y+2^{-k}$ gives
\[
(2\pi)^m\int_{\R^2}|u-v|^m|\Psi(u,v)|\,d(u,v)
\le2^8C_{\ref{lem:theta_prim},4}^2 2^{(2-\gamma)k}
\le c_1.
\]
...
\begin{lemma}[constant $C_{\ref{lem:leftbump1_long}}$ \auto]\label{constant left bump one long}\using{constant Whitney reduction}\using{constant theta primitive}\using{lem:leftbump1_long}
\begin{equation}\label{constant left bump one long bound}
C_{\ref{lem:leftbump1_long}}\le2^{705}.
\end{equation}
\end{lemma}
\begin{proof}[Proof \auto]
Use the sharper estimate from Lemma \ref{constant Whitney reduction} and Lemma \ref{constant theta primitive} with $N=4$ in \eqref{auto:left-bump-one-long-constant-definition}. This gives
\[
C_{\ref{lem:leftbump1_long}}<\tfrac{11}{18}2^{705}<2^{705}.
\]
\end{proof}


\begin{lemma}\label{lem:leftbump1}\using{lem:rescaling}\using{lem:shortlongjumps}\using{lem:leftbump1_long}\using{lem:leftbump1_short1}\using{lem:leftbump1_short2}\using{lem:ftccs-R}\using{lem:ftc_ATphi}
...
For each fixed $t\in [1,2]$ we use Lemma \ref{lem:leftbump1_short1} and Lemma \ref{lem:leftbump1_short2} and that $\#L\le J+1$.
...
\begin{lemma}[constant $C_{\ref{lem:leftbump1}}$ \auto]\label{constant left bump one}\using{constant left bump one short one}\using{constant left bump one short two}\using{lem:leftbump1_short1}\using{lem:leftbump1_short2}\using{constant left bump one long}\using{lem:leftbump1}\using{lem:smoothingdecomp}\using{lem:norm_A_sum_le_sum}\using{lem:mainbump1}\using{lem:mainbump2}\using{lem:leftbump}\using{constant main bump one}\using{constant main bump two}\using{constant left bump}\using{thm:nct main real}
\begin{equation}\label{constant left bump one bound}
C_{\ref{lem:leftbump1}}\le2^{715}.
\end{equation}
\end{lemma}
\begin{proof}[Proof \auto]
The sharper estimates in Lemmas \ref{constant left bump one short one} and \ref{constant left bump one short two} give
\[
2^7C_{\ref{lem:leftbump1_short1}}^{1/2}C_{\ref{lem:leftbump1_short2}}^{1/2}
<\tfrac{11}{18}2^{715}.
\]
Lemma \ref{constant left bump one long} bounds the second term in the defining sum by $2^{706}$. Hence
\[
C_{\ref{lem:leftbump1}}
<\left(\tfrac{11}{18}+\tfrac1{512}\right)2^{715}<2^{715}.
\]
\end{proof}
...
\[
C_{\mathrm{final}}
<\tfrac78 2^{769}+2^{677}+2^{714}+2^{723}
<2^{769}=C_{\ref{thm:nct main real}}.
\]
\end{proof}
