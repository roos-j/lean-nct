...
the ERC Synergy Grant 101224275 during research stays at Bonn. PD was supported in part by NSF grant DMS-2554859 and a grant from the Simons Foundation. JR was supported in part by NSF grants DMS-2555784 and DMS-2154835 and a grant from the Simons Foundation.
...
\lean{increaseData_implies_diagonalBand}
\leanok
...
\lean{inductPositiveTerms_implies_increaseData}
\leanok
Let $k\in\N$ with $1\le k\le n-1$ and $C\in [1,\infty)$.
...
\lean{inductPositiveTermsByInduction}
\leanok
...
\lean{inductPositiveTermsTheorem}
\leanok
...
\begin{lemma}[Existence of a universal pair]\label{lem:cpair}\lean{existsUniversalPair}\leanok\uses{standard bump properties}\usesdefs{standard bump,def:cn-window,def:cpair,def:unipair}
...
\begin{lemma}\label{lem:bumpbasic}\lean{bumpBasic}\leanok\uses{}\usesdefs{def:cn-window,def:unipair,defn:window based bump functions}
...
\lean{absDerivFourierPhiThreeLe}
\leanok
...
\lean{constantPhiThreeDerivative}
\leanok
For $m=0$,
...
\lean{absDerivFourierTPhiThreeLe}
\leanok
...
\lean{constantTPhiThreeDerivative}
\leanok
For $m=0$,
...
\lean{constantRhoKernelsReduction}
\leanok
...
\lean{affineDiagonalCancellationReduction}
\leanok
...
\lean{increaseDataBracketDomination}
\leanok
Let \(a\in A\), and let
$
...
\lean{constantIncreaseDataBracketDomination}
\leanok
...
\lean{increaseDataGaussianExpansion}
\leanok
...
\lean{constantIncreaseDataGaussianExpansion}
\leanok
...
\lean{constantIncreaseDataReduction}
\leanok
...
\lean{constantDiagonalBandReduction}
\leanok
...
\lean{vanishingDiagonalReduction}
\leanok
...
\lean{inductPositiveTermsReductionNonWhitney}
\leanok
Let $a\in A$ and let
...
\lean{constantNonWhitneyReduction}
\leanok
...
\lean{inductPositiveTermsReductionNonWhitneySkip}
\leanok
Let $a\in A$ and let
...
\lean{constantNonWhitneySkipReduction}
\leanok
...
\lean{inductPositiveTermsReductionWhitneyGap}
\leanok
...
\lean{constantWhitneyGapReduction}
\leanok
...
\lean{inductPositiveTermsReductionWhitney}
\leanok
...
\lean{inductPositiveTermsReductionWhitneyProduct}
\leanok
Let $a\in A$ and let
...
\lean{constantWhitneyProductReduction}
\leanok
...
\lean{constantMainBumpOneLongOne}
\leanok
...
\lean{constantMainBumpOneLongTwo}
\leanok
...
\lean{constantLeftBumpOneShortOne}
\leanok
...
\lean{constantLeftBumpOneShortTwo}
\leanok
...
\begin{proposition}[Permutation of the endpoint estimate]\label{permutation of endpoint estimate}\lean{permutation_of_endpoint_estimate}\leanok\uses{thm:nct main real}\usesdefs{A_def}
...
\begin{exttheorem}[Multilinear interpolation]\label{multilinear interpolation external}\lean{multilinear_interpolation_external}\leanok\uses{}\usesdefs{}
...
\begin{proposition}[Range of exponents]\label{exponent polytope}\lean{exponent_polytope,exponent_polytope_of_exponents}\leanok\uses{}\usesdefs{}
...
\begin{proposition}[Interpolated real-variable estimate]\label{interpolated real variable estimate}\lean{interpolated_real_variable_estimate}\leanok\uses{permutation of endpoint estimate,exponent polytope,multilinear interpolation external}\usesdefs{A_def}
...
\begin{exttheorem}[Calder\'on transference principle]\label{Calderon transference external}\lean{calderon_transference_external}\leanok\uses{}\usesdefs{ergodic averages,A_def}
...
\begin{theorem}[Ergodic jump estimate with general exponents]\label{thm:ergodicthm-interpolation}\lean{ergodic_jump_estimate_interpolation}\leanok\uses{interpolated real variable estimate,Calderon transference external}\usesdefs{ergodic averages}
...
\begin{theorem}[Symmetric ergodic jump estimate]\label{thm:ergodicthmJ}\lean{symmetric_ergodic_jump_estimate}\leanok\uses{thm:ergodicthm-interpolation}\usesdefs{ergodic averages}
...
\begin{lemma}[From jump estimates to variation]\label{jump estimates imply variation}\lean{jump_estimates_imply_variation_enorm,jump_estimates_imply_variation_endpoint_enorm}\leanok\uses{}\usesdefs{}
