...
\date{\today}
\subjclass[2020]{Primary 42B20, 68V20; Secondary 42B15, 37A30.}
...
\begin{center}
    ...
    This is the document of PASS 9.}}
\end{center}
...
The main result concerns norm-variation estimates for multiple ergodic averages associated with $n\ge 2$ commuting measure preserving transformations, providing a quantitative strengthening of Tao's norm-convergence theorem. At the core of the analysis lies an explicit real-variable estimate for twisted multilinear averages that is closely related to certain singular Brascamp--Lieb inequalities.   \end{abstract}
...
Recall that that $T: X \to X$ is a {\em measure preserving transformation} if for every measurable $A\subset X$ the set $T^{-1}(A)$ is measurable and has the
same measure as $A$.
...
\end{theorem}
(See Definition \ref{variation seminorm on positive integers} for the definition of the $r$-variation norm.)

Since finite variation of a sequence implies convergence of the sequence and since $L^\infty(X)\subset L^{2n}(X)$ if $X$ is a probability space, Theorem \ref{thm:ergodicthm} immediately recovers Tao's theorem  \cite[Theorem 1.1]{Tao07} for $n\ge 2$, which concerned {\em qualitative} norm convergence. Ergodic-theoretic proofs of Tao's theorem were also given by Austin \cite{Austin10}, Host \cite{Host09}, and a generalization to nilpotent groups was proved in \cite{Walsh12}.
Norm-convergence for a single ergodic average ($n=1$) for $r\ge 2$ was proved by Jones--Ostrovskii--Rosenblatt \cite{JonesOstrovskiiRosenblatt96}. 
Theorem \ref{thm:ergodicthm} resolves an open problem that was stated by Avigad and Rute in the closing section of \cite{AvigadRute15}. 
...
Theorem~\ref{thm:ergodicthm} has been reduced by the Calder\'on transference principle to Theorem~\ref{thm:nct main real} below --- extending the cases $n = 2$ and $n = 3$ --- by Pernegger \cite{Pernegger2025}, who also formalized this reduction. \jr{discuss} Theorem~\ref{thm:nct main real} is an {\em a priori} estimate over the real numbers. The proof of this latter estimate is the heart of the matter of this paper. It is closely related to the study of certain singular Brascamp--Lieb inequalities in real harmonic analysis, which can be viewed as generalizations of Kova\v{c}'s twisted paraproduct \cite{Kovac12}.  Also see the survey \cite{DurcikThiele21survey} and \cite{DurcikKovacThiele19,DurcikThiele20, DurcikSlavikovaThiele22}. 

\pd{I would suggest to explicitly mention cancellation estimates for the simplex Hilbert transform rather than the twisted paraproduct (we can still include it in SBL literature, of course). These  averages  lead to simplex HT and I think we should mention the operator of the correct complexity (twisted paraproduct is associated with smoother averages, the so-called cubic averages)
something short like this: 

The proof of this latter estimate is the heart of the matter of this paper.  It is closely related to the study of cancellation estimates for the simplex Hilbert transform \cite{DurcikKovacThiele19}  and  the singular Brascamp--Lieb estimates  \cite{Kovac12,DurcikThiele20, DurcikSlavikovaThiele22} in real harmonic analysis.}\jr{Yes please implement change}

\begin{definition}[Twisted average]\lean{nCT.twistedAverage,nCT.rescaledTwistedAverage}
Let $n\in\mathbb{N}$. For an $n$-tuple of real-valued functions $\mathbf f = (f_i)_{i\in \mathbb{N}, i<n}$ on $\R^n$, a function $\chi:\R\to\R$ and $x\in\mathbb{R}^n$ denote
...
\end{equation}
for $i\in[n)$, we have 
% that the function $A_{t_{j+1}}(\mathbf 1_{[0,1]},\mathbf f) -
%   A_{t_j}(\mathbf 1_{[0,1]},\mathbf f)$ is measurable with finite $L^2(\R^n)$ norm for every $j\in [J)$, and
...
After this blueprint was completed, its contents were formally verified using the proof assistant Lean 4 \cite{LeanPaper} and its mathematical library \texttt{mathlib} \cite{Mathlib}. The formalization is available at 
\begin{center}
\href{https://github.com/roos-j/lean-nct}{https://github.com/roos-j/lean-nct}
\end{center}
The formalization made heavy use of emerging autoformalization capabilities of current frontier models, specifically OpenAI's \texttt{Codex/5.6-terra-ultra}. While the statements of Theorem \ref{thm:ergodicthm} and Theorem \ref{thm:nct main real} and its associated definitions have been carefully formalized by hand, virtually all of the remaining formalization was machine-generated across many extended sessions during a one-week period, with a custom instruction set and careful human supervision. The timeline and required amount of human effort are in striking contrast to those of previous large formalization projects in analysis such as the Carleson project \cite{CarlesonBlueprint}, \cite{CarlesonPaper}, which was predominantly human-generated and concluded its main objectives \pd{maybe remove "its main objectives"?}\jr{no, because the Carleson project still has pending objectives, refactoring, upstreaming} \pd{OK, I just thought "main objectives" sounded AI-like} before the current set of highly formalization-capable models was made publicly available.
...
\subsection{Blueprint conventions}
...
\item {\em Mathlib awareness.} Generally, only facts that are currently available in mathlib in some shape or form can pass as `standard theorems` that require no proof. %As a consequence, this blueprint proves various facts, that could otherwise be considered `standard`, such as elementary properties of Wiener spaces and various properties of bump functions.
We make two exceptions to this rule: an instance of the Calder\'on transference principle (Theorem \ref{Calderon transference external}, and a multilinear complex interpolation theorem (Theorem \ref{multilinear interpolation external}). These were essentially already formalized prior to this formalization project (the Calder\'on transference principle was hand-formalized by Pernegger \cite{Pernegger2025}; complex interpolation was autoformalized by one of the authors).
\item {\em Explicit dependencies.} Whenever a named theorem is used in a proof it must be referred to explicitly. The parameters of the theorem should also usually be made explicit, unless it is clear from context. This includes proofs of hypotheses.
...
\lean{Codex.bracketBump,Codex.scaledBracketBump,Codex.scaledBracketBumpReal}
Define
...
\lean{Codex.localSupremumMeasurability}
\leanok
...
\lean{Codex.wienerEnvelope,Codex.MemW0,Codex.wienerNorm,Codex.wienerNormOne}
...
\lean{Codex.wienerNorm_le_max_one_three_mul_div_pow_mul,Codex.memW0_iff_integrable_wienerEnvelope}
\leanok
Let $d\in \N$.
...
\lean{Codex.exists_wienerEnvelope_fiber_and_integral_comp_injective_continuousLinearMap_bound}
\leanok
...
\lean{Codex.MemW0.fintype_tensor,Codex.fintype_tensor_wienerNorm_le}
\leanok
...
\lean{Codex.exists_brascamp_lieb_memW0}
\leanok
...
\lean{Codex.convolutionAlongVector}
...
\lean{Codex.memW0_convolutionAlongVector,Codex.fourier_convolutionAlongVector}
\leanok
...
\lean{Codex.normalizedFunctionTuples}
...
\lean{Codex.cubeBrascampLiebForm}
...
\lean{Codex.prismBrascampLiebForm}
...
\lean{Codex.simplificationOnePrism}
\leanok
...
\lean{Codex.singlyCancellativeKernel_memW0,Codex.singlyCancellativeLift_memW0,Codex.singlyCancellativeCauchySchwarz_bound}
\leanok
...
\lean{Codex.doublyCancellativeKernel_memW0,Codex.doublyCancellativeLift_memW0,Codex.doublyCancellativeCauchySchwarz_bound}
\leanok
...
\lean{Codex.positivityKernel_memW0,Codex.positivityKernel_nonnegative}
\leanok
...
\lean{Codex.mToK,Codex.mToK_integrand_memW0,Codex.mToK_memW0,Codex.mToK_eLpNorm_one_le}
\leanok
...
\lean{Codex.positivityM_memW0,Codex.positivityM_nonnegative}
\leanok
...
\lean{Codex.cauchySchwarzKernel_memW0,Codex.cauchySchwarzLift_memW0,Codex.cauchySchwarzAtK_bound}
\leanok
...
\lean{Codex.doublyCauchySchwarzKernel_memW0,Codex.cauchySchwarzLift_memW0,Codex.cauchySchwarzAtNMinusOne_bound,Codex.cauchySchwarzAtNMinusOne}
\leanok
...
\lean{Codex.SpacedSequence,Codex.A}
...
\lean{Codex.extensionOfSequences}
\leanok
...
\lean{Codex.max_mem_A,Codex.smul_mem_A,Codex.shift_mem_A,Codex.sqrt_sq_add_sq_mem_A}
\leanok
...
\lean{Codex.WithinSequenceDistance,Codex.SequenceDistance}
...
\lean{Codex.sequenceDistance_zero_eq,Codex.sequenceDistance_comm,Codex.sequenceDistance_triangle,Codex.sequenceDistance_shift_le,Codex.sequenceDistance_smul,Codex.sequenceDistance_pow_two_smul_le}
\leanok
    Let $a,b,c\in {\rm A}$.
    Then
...
\lean{Codex.sequenceDistanceBall}
...
\lean{Codex.sqrtOneMinusGaussian,Codex.sqrtOneMinusGaussian_wellDefined,Codex.continuous_sqrtOneMinusGaussian,Codex.sqrtOneMinusGaussian_lower,Codex.sqrtOneMinusGaussian_bounds}
\leanok
    The function
...
\lean{Codex.gaussian_memSchwartz,Codex.gaussian_memW0,Codex.gaussian_fourier_fixed,Codex.gaussianRescale_convolution,Codex.gaussianRescale_fourier}
\leanok
...
\lean{Codex.poissonKernel,Codex.poissonKernel_fourier}
\leanok
Let
...
\lean{Codex.auxiliaryFunctionB_properties}
\leanok
...
\lean{Codex.meanValueBumpEstimate}
\leanok
...
\lean{Codex.standardBumpFinite,Codex.standardBump,Codex.standardBumpRescale}
...
\lean{Codex.standardBumpProperties,Codex.standardBumpProperties_l1Convergence,Codex.standardBumpProperties_linfConvergence,Codex.standardBumpProperties_schwartz,Codex.standardBumpProperties_fourierShape,Codex.standardBumpProperties_fourierDerivativeEstimate,Codex.standardBumpProperties_derivativeDecay}
\leanok
...
\lean{Codex.orthogonalDomination}
\leanok
...
\lean{Codex.diagonalSquareRoot,Codex.diagonalSquareRoot_bound,Codex.diagonalSquareRoot_memW0}
\leanok
...
\lean{Codex.constantDiagonalSquareRoot}
\leanok
For every $N\ge1$,
...
\lean{Codex.derivativeDiagonalSquareRoot_differentiable,Codex.derivativeDiagonalSquareRoot_bound}
\leanok
...
\lean{Codex.constantDerivativeDiagonalSquareRoot}
\leanok
For every $N\ge1$,
...
\lean{Codex.gaussianBumpEstimate}
\leanok
...
\lean{Codex.constantGaussianBumpEstimate}
\leanok
For every $N\in\N$,
...
\lean{Codex.derivativeEstimateForG}
\leanok
...
\lean{Codex.secondGaussianEstimate}
\leanok
...
\lean{Codex.constantSecondGaussianEstimate}
\leanok
For every $N\in\N$,
...
\lean{Codex.gaussianBumpDecomposition}
\leanok
...
\lean{Codex.fourScaleGaussianKernel}
\leanok
...
\lean{Codex.constantFourScaleGaussianKernel}
\leanok
For every $N\in\N$,
...
\lean{Codex.meanFourScaleGaussianKernel}
\leanok
Let $c>0$, $N\in \mathbb N$ with $N\geq 2$ and assume that $\phi$ is a $W_0(\R)$ function such that
$\widehat{\phi}$ is supported in $[-1,1]$ and equal to $1$ on $[-1/2,1/2]$ and $\widehat{\phi}$ is $N$ times continuously differentiable and $\max_{0\leq m \leq N} \|\widehat{\phi}^{(m)}\|_{\infty} \leq c$.
...
\lean{Codex.constantMeanFourScaleGaussianKernel}
\leanok
For every $N\in\N$,
...
\lean{Codex.GeometricParameters,Codex.sequencePairDistance,Codex.geometricDelta}
...
\lean{Codex.DoubleSequence,Codex.MemDoubleSequence}
...
\lean{Codex.KernelSequence,Codex.MemKernelSequence,Codex.kernelSequenceSeminorm}
...
\lean{Codex.twoDimensionalGaussian,Codex.gammaGaussian}
...
\lean{Codex.squareRootGaussianDifference}
...
\lean{Codex.squareRootGaussianDifference_memW0,Codex.sMultiplier_memW0}
\leanok
...
\lean{Codex.hMultiplier_memDoubleSequence}
\leanok
...
\lean{Codex.hMultiplier_fourier_diagonal_vanishing}
\leanok
...
\lean{Codex.hMultiplier_vanishing_integral}
\leanok
...
\lean{Codex.lMultiplierAtScale_memDoubleSequence,Codex.lMultiplierAtScale_tendsto_hMultiplier}
\leanok
...
\lean{Codex.lMultiplierAtScale_tendsto_zero}
\leanok
...
\lean{Codex.multiplierIndexSet,Codex.lMultiplier}
Let $\gamma=(k,u,a)\in \Gamma$.
...
\lean{Codex.sumOverMultiplierIndex}
...
\lean{Codex.sumLMultiplierConvergenceL1}
\leanok
...
\lean{Codex.prismSumLeSumPrismL1}
\leanok
...
\lean{Codex.sigmaMultiplier,Codex.nMultiplier}
...
\lean{Codex.nKernelWellDefinedness}
\leanok
...
\lean{Codex.hKernelEstimateGaussianDomination}
\leanok
...
\lean{Codex.constantHKernelEstimateGaussianDomination}
\leanok
...
\lean{Codex.hKernelDerivativeEstimateGaussianDomination}
\leanok
...
\lean{Codex.constantHKernelDerivativeEstimateGaussianDomination}
\leanok
...
\lean{Codex.gaussianDominationCombined}
\leanok
Set
...
\lean{Codex.gaussDominationCase1}
\leanok
...
\lean{Codex.gaussDominationCase2}
\leanok
The conclusion of Proposition \ref{Gaussian domination combined} holds in the case $\iota=(h,0)$, $h<0$, with the constant in \eqref{Gaussian domination main estimate} equal to
...
\lean{Codex.gaussDominationCase3}
\leanok
...
\lean{Codex.gaussDominationConstant}
\leanok
We have
...
\lean{Codex.InductPositiveTerms}
...
\lean{Codex.vanishingDiagonal_implies_inductPositiveTerms}
\leanok
Let $k\in\N$ with $1\le k\le n$ and $C\in [1,\infty)$.
...
\lean{Codex.diagonalBand_implies_vanishingDiagonal}
\leanok
Let $k\in\N$ with $1\le k\le n-1$ and $C\in [1,\infty)$.
...
\lean{Codex.vanishingKernelIntegral}
\leanok
...
\lean{Codex.increaseData_implies_diagonalBand}
\leanok
...
\lean{Codex.inductPositiveTerms_implies_increaseData}
\leanok
Let $k\in\N$ with $1\le k\le n-1$ and $C\in [1,\infty)$.
...
\lean{Codex.constantInductPositiveTermsImplyIncreaseData}
\leanok
...
\lean{Codex.inductPositiveTermsByInduction}
\leanok
...
\lean{Codex.inductPositiveTermsTheorem}
\leanok
...
\lean{Codex.constantInductPositiveTermsTheorem}
\leanok
...
\lean{Codex.fourierDerivativeMul}
\leanok
...
\lean{Codex.constantOffCenterBump}
\leanok
...
\begin{lemma}[Existence of a universal pair]\label{lem:cpair}\lean{Codex.existsUniversalPair}\leanok\uses{standard bump properties}\usesdefs{standard bump,def:cn-window,def:cpair,def:unipair}
...
\lean{Codex.windowBasedBumpFunctions}
...
\begin{lemma}\label{lem:bumpbasic}\lean{Codex.bumpBasic}\leanok\uses{}\usesdefs{def:cn-window,def:unipair,defn:window based bump functions}
...
\lean{Codex.absDerivFourierPhiThreeLe}
\leanok
...
\lean{Codex.constantPhiThreeDerivative}
\leanok
For $m=0$,
...
\lean{Codex.absDerivFourierTPhiThreeLe}
\leanok
...
\lean{Codex.constantTPhiThreeDerivative}
\leanok
For $m=0$,
...
\lean{Codex.constantThetaPrimitive}
\leanok
For $2\le N<N_{\ref{def:unipair}}$,
...
\lean{Codex.constantPhiJProposition}
\leanok
...
\lean{Codex.rhoKernelsReduction}
\leanok
...
\lean{Codex.constantRhoKernelsReduction}
\leanok
...
\lean{Codex.affineDiagonalCancellationReduction}
\leanok
...
\lean{Codex.increaseDataBracketDomination}
\leanok
Let \(a\in A\), and let
$
...
\lean{Codex.constantIncreaseDataBracketDomination}
\leanok
...
\lean{Codex.increaseDataGaussianExpansion}
\leanok
...
\lean{Codex.constantIncreaseDataGaussianExpansion}
\leanok
...
\lean{Codex.increaseDataReduction}
\leanok
...
\lean{Codex.constantIncreaseDataReduction}
\leanok
...
\lean{Codex.diagonalBandReduction}
\leanok
...
\lean{Codex.constantDiagonalBandReduction}
\leanok
...
\lean{Codex.vanishingDiagonalReduction}
\leanok
...
\lean{Codex.oneScaleEstimateWindow}
\leanok
...
\lean{Codex.constantOneScaleWindow}
\leanok
...
\lean{Codex.fourierTransformWindow}
\leanok
...
\lean{Codex.inductPositiveTermsReductionNonWhitney}
\leanok
Let $a\in A$ and let
...
\lean{Codex.constantNonWhitneyReduction}
\leanok
...
\lean{Codex.inductPositiveTermsReductionNonWhitneySkip}
\leanok
Let $a\in A$ and let
...
\lean{Codex.constantNonWhitneySkipReduction}
\leanok
...
\lean{Codex.inductPositiveTermsReductionWhitneyGap}
\leanok
...
\lean{Codex.constantWhitneyGapReduction}
\leanok
...
\lean{Codex.inductPositiveTermsReductionWhitney}
\leanok
...
\lean{Codex.constantWhitneyReduction}
\leanok
...
\lean{Codex.inductPositiveTermsReductionWhitneyProduct}
\leanok
Let $a\in A$ and let
...
\lean{Codex.constantWhitneyProductReduction}
\leanok
...
\lean{Codex.constantMainAuxiliaryOne}
\leanok
...
\lean{Codex.shortLongFtcReduction}
\leanok
...
\lean{Codex.constantMainBumpOneLongOne}
\leanok
...
\lean{Codex.constantMainBumpOneLongTwo}
\leanok
...
\lean{Codex.constantMainBumpOneLong}
\leanok
...
\lean{Codex.constantMainBumpOne}
\leanok
...
\lean{Codex.constantMainAuxiliaryTwo}
\leanok
...
\lean{Codex.constantMainBumpTwo}
\leanok
...
\lean{Codex.leftBumpOneShortOne}
\leanok
...
\lean{Codex.constantLeftBumpOneShortOne}
\leanok
...
\lean{Codex.leftBumpOneShortTwo}
\leanok
Let $\gamma=\frac12$. For every $k\le-1$ and every strictly increasing sequence of integers $(k_j)_{j\in[J)}$,
...
\lean{Codex.constantLeftBumpOneShortTwo}
\leanok
...
\lean{Codex.constantLeftBumpOneLong}
\leanok
...
\lean{Codex.constantLeftBumpOne}
\leanok
...
\begin{proposition}[Permutation of the endpoint estimate]\label{permutation of endpoint estimate}\lean{Codex.permutation_of_endpoint_estimate}\leanok\uses{thm:nct main real}\usesdefs{A_def}
...
\begin{exttheorem}[Multilinear interpolation]\label{multilinear interpolation external}\lean{Codex.multilinear_interpolation_external}\leanok\uses{}\usesdefs{}
...
\begin{proposition}[Range of exponents]\label{exponent polytope}\lean{Codex.exponent_polytope,Codex.exponent_polytope_of_exponents}\leanok\uses{}\usesdefs{}
...
\begin{proposition}[Interpolated real-variable estimate]\label{interpolated real variable estimate}\lean{Codex.interpolated_real_variable_estimate}\leanok\uses{permutation of endpoint estimate,exponent polytope,multilinear interpolation external}\usesdefs{A_def}
...
\begin{exttheorem}[Calder\'on transference principle]\label{Calderon transference external}\lean{Codex.calderon_transference_external}\leanok\uses{}\usesdefs{ergodic averages,A_def}
...
\begin{theorem}[Ergodic jump estimate with general exponents]\label{thm:ergodicthm-interpolation}\lean{Codex.ergodic_jump_estimate_interpolation}\leanok\uses{interpolated real variable estimate,Calderon transference external}\usesdefs{ergodic averages}
...
\begin{theorem}[Symmetric ergodic jump estimate]\label{thm:ergodicthmJ}\lean{Codex.symmetric_ergodic_jump_estimate}\leanok\uses{thm:ergodicthm-interpolation}\usesdefs{ergodic averages}
...
\begin{lemma}[From jump estimates to variation]\label{jump estimates imply variation}\lean{Codex.jump_estimates_imply_variation_enorm,Codex.jump_estimates_imply_variation_endpoint_enorm}\leanok\uses{}\usesdefs{}
