...
\begin{center}
    ...
    This is the document of PASS 6.}}
\end{center}
...
\begin{theorem}[Main twisted theorem]\label{thm:nct main real}\uses{lem:smoothingdecomp,lem:norm_A_sum_le_sum,lem:mainbump1,lem:mainbump2,lem:leftbump,lem:leftbump1,constant main bump one,constant main bump two,constant left bump,constant left bump one}\usesdefs{A_def}
...
\begin{proposition}[mean value bump estimate]\label{mean value bump estimate 2}\uses{lem:smoothdecay2}\usesdefs{bracket bump,auto:Wiener-space-definition}
...
C_{\ref{diagonal square root},2}=3264\sqrt2<2^{13}.
\end{equation}
\end{lemma}
\begin{proof}[Proof \auto]
...
C_{\ref{derivative of diagonal square root},2}=36864\sqrt2<2^{16}.
\end{equation}
\end{lemma}
\begin{proof}[Proof \auto]
...
C_{\ref{L:second-gaussian-estimate},0}<46656<2^{16},
\qquad
C_{\ref{L:second-gaussian-estimate},1}<3040704<2^{22},
\end{equation}
\begin{equation}\label{auto:constant-second-gaussian-estimate-small-high}
C_{\ref{L:second-gaussian-estimate},2}<159359088384<2^{38},
\qquad
C_{\ref{L:second-gaussian-estimate},3}<767070519557262336<2^{60}.
\end{equation}
\end{lemma}
\begin{proof}[Proof \auto]
...
C_{\ref{four scale Gaussian kernel},2}<637436354528<2^{40},
\qquad
C_{\ref{four scale Gaussian kernel},3}<6136564156458101504<2^{63}.
\end{equation}
\end{lemma}
\begin{proof}[Proof \auto]
...
\begin{lemma}[constant $C_{\ref{mean four scale Gaussian kernel},N}$ \auto]\label{constant mean four scale Gaussian kernel}\uses{L:gaussian-bump-estimate,L:second-gaussian-estimate,constant gaussian bump estimate,constant second gaussian estimate,mean four scale Gaussian kernel}\usesdefs{}
For every $N\in\N$,
...
\end{lemma}
\begin{proof}[Proof \auto]
Substitution of the defining estimates from Lemmas \ref{L:gaussian-bump-estimate} and \ref{L:second-gaussian-estimate} proves the first bound. For $N=2$, use the finite estimates in Lemmas \ref{constant gaussian bump estimate} and \ref{constant second gaussian estimate}. In the defining constant from Proposition \ref{mean four scale Gaussian kernel}, specifically the second maximum in \eqref{auto:mean-four-scale-Gaussian-constant}, the term with $l=2$ is the largest. Substitution gives
\[
C_{\ref{mean four scale Gaussian kernel},2}<20397963318112<2^{45}.
\]
\end{proof}
...
\begin{lemma}\label{L:F_t}\uses{P:schwartz-into-wiener,convolution vector,H-in-X}\usesdefs{standard bump,geometric parameters,double sequence of 2D functions,H multiplier,auto:L-kernel-definition}
...
\begin{lemma}\label{L:ft-infty}\uses{H vanishing integral}\usesdefs{standard bump,geometric parameters,H multiplier,auto:L-kernel-definition}
...
\begin{proposition}[Properties of L multipliers]\label{sum L multiplier convergence-L1}\uses{P:schwartz-into-wiener,convolution vector,H-in-X,L:F_t,L:ft-infty}\usesdefs{auto:Wiener-space-definition,geometric parameters,double sequence of 2D functions,H multiplier,auto:L-kernel-definition,L multiplier,summation-definition}
...
\begin{lemma}\label{sandwich sums L1}\uses{}\usesdefs{geometric parameters,double sequence of 2D functions,2D Gaussians,sandwich kernel,L multiplier,summation-definition}
Let $\gamma=(k,u,a)\in \Gamma$.
...
\begin{lemma}\label{prism sum le sum prism-L1}\uses{prism BL inequality,M to K}\usesdefs{auto:Wiener-space-definition,normalized function tuples,auto:prism-form-definition,geometric parameters,L multiplier,summation-definition}
Let $\gamma=(k,u,a)\in \Gamma$.
...
\begin{proposition}\label{auto:N-kernel-well-definedness}\uses{four scale Gaussian kernel,H-in-X,convolution vector}\usesdefs{auto:Wiener-space-definition,auto:convolution-along-vector-definition,geometric parameters,double sequence of 2D functions,H multiplier,L multiplier,N multiplier}
...
\begin{proposition}[Kernel estimate for Gaussian domination]\label{H kernel estimate Gaussian domination}\uses{Operations on spaced sequences,Properties of distance of sequences,diagonal square root,Gaussian bump decay}\usesdefs{gaussian,bracket bump,multiplicatively spaced monotone sequences,Distance of spaced sequences,closed balls in A,geometric parameters,auto:unitary-matrices-definition,2D Gaussians,auto:Gaussian-difference-kernel-definition,square root Gaussian difference,s multiplier,H multiplier}
...
\begin{lemma}[constant $C_{\ref{H kernel estimate Gaussian domination}}$ \auto]\label{constant H kernel estimate Gaussian domination}\uses{constant diagonal square root,diagonal square root,Gaussian bump decay,H kernel estimate Gaussian domination}\usesdefs{}
...
\begin{proposition}[Kernel derivative estimate for Gaussian domination]\label{H kernel derivative estimate Gaussian domination}\uses{H kernel estimate Gaussian domination,diagonal square root,derivative of diagonal square root,Gaussian bump decay}\usesdefs{bracket bump,closed balls in A,geometric parameters,auto:unitary-matrices-definition,2D Gaussians,auto:Gaussian-difference-kernel-definition,s multiplier,H multiplier}
Let $\gamma=(k,u,a)\in \Gamma$ and let $i\in [k)$.
...
\begin{lemma}[constant $C_{\ref{H kernel derivative estimate Gaussian domination}}$ \auto]\label{constant H kernel derivative estimate Gaussian domination}\uses{constant diagonal square root,constant derivative diagonal square root,diagonal square root,derivative of diagonal square root,Gaussian bump decay,H kernel derivative estimate Gaussian domination}\usesdefs{}
...
\begin{lemma}\label{lem:theta_decay}\uses{lem:smoothdecay2}\usesdefs{bracket bump,def:cn-window,def:unipair,defn:window based bump functions}
Let $1\le N\le3$. Then
...
C_{\ref{lem:abs_deriv_ft_phi3_le},3}=1104C_{\ref{def:unipair}}^2+2350C_{\ref{def:unipair}}+2048.
\end{equation}
\end{lemma}
\begin{proof}[Proof \auto]
...
C_{\ref{lem:abs_deriv_ft_Tphi3_le},2}=1296C_{\ref{def:unipair}}^2+2630C_{\ref{def:unipair}}+2220.
\end{equation}
\end{lemma}
\begin{proof}[Proof \auto]
...
C_{\ref{lem:rho-kernels-reduction}}
<44114430494276321280<\tfrac35 2^{66}<2^{66}.
\end{equation}
\end{lemma}
\begin{proof}[Proof \auto]
...
\begin{lemma}\label{L:fourier-transform-window}\uses{}\usesdefs{multiplicatively spaced monotone sequences,def:cn-window,def:cpair,def:unipair,defn:window based bump functions}
...
where $C_{\ref{P:induct-positive-terms-reduction-non-whitney}}=C_{\ref{P:one-scale-estimate-window}}+ 64 \max(1,(C_{\ref{def:unipair}}/2\pi)^4) C_{\ref{lem:smoothdecay2},2}^2C_{\ref{P:diagonal-band-reduction}}$.
\end{proposition}
...
\[
64 C_{\ref{lem:smoothdecay2},2}^2 \max(1,(C_{\ref{def:unipair}}/2\pi)^4)
...
\[
\|\mathbf{M}_1\|_{\rm M(1)} \leq 64 \max(1,(C_{\ref{def:unipair}}/2\pi)^4) C_{\ref{lem:smoothdecay2},2}^2C_{\ref{P:diagonal-band-reduction}}.
\]
\end{proof}
...
\[
2^{10}\left(\tfrac{2^{15}}6\right)^4\tfrac{63}{64}2^{480}
=\tfrac79 2^{540}.
\]
The first term is $2^{39}<\frac19 2^{540}$, which proves the claim.
\end{proof}
...
\]
Apply Proposition \ref{P:induct-positive-terms-reduction-whitney-product} with $2^{-2}\psi_{(t)}$ in place of $\psi$, and multiply the resulting estimate by $2^4$. The Fourier support condition follows from \eqref{main_aux1_supp} and $t\in[1,2]$. Finally,
\[
...
C_{\ref{lem:main_aux1}}<\tfrac{1397}{2048}2^{573}<2^{573}.
\end{equation}
\end{lemma}
\begin{proof}[Proof \auto]
...
C_{\ref{lem:mainbump1_long1}}<\tfrac{1397}{2048}2^{605}<2^{605}.
\end{equation}
\end{lemma}
\begin{proof}[Proof \auto]
...
C_{\ref{lem:mainbump1_long2}}<\tfrac89 2^{542}<2^{542}.
\end{equation}
\end{lemma}
\begin{proof}[Proof \auto]
...
\end{lemma}
\begin{proof}[Proof \auto]
Use Lemmas \ref{constant main bump one long one} and \ref{constant main bump one long two} in the defining sum. The first contribution after multiplication by $2$ is smaller than $\frac{1397}{2048}2^{606}$, while the second is smaller than $\frac89 2^{543}$. Their sum is smaller than $\frac{11}{16}2^{606}$.
\end{proof}
...
C_{\ref{lem:main_aux2}}<\tfrac{4191}{8192}2^{578}<2^{578}.
\end{equation}
\end{lemma}
\begin{proof}[Proof \auto]
...
\begin{lemma}\label{lem:leftbump1_short1}\uses{A to Lambda,lem:theta_prim,lem:thetat_offcenter,P:induct-positive-terms-reduction-whitney,lem:int_fct,lem:phi4_supp}\usesdefs{A_def,bracket bump,normalized function tuples,auto:prism-form-definition,defn:window based bump functions}
...
C_{\ref{lem:leftbump1_short1}}
<\tfrac{185801}{262144}2^{628}<\tfrac{23}{32}2^{628}<2^{628}.
\end{equation}
\end{lemma}
\begin{proof}[Proof \auto]
...
\begin{lemma}\label{lem:leftbump1_short2}\uses{A to Lambda,lem:theta_prim,lem:theta_decay,lem:thetat_offcenter,P:induct-positive-terms-reduction-whitney,lem:int_fct,lem:phi4_supp,lem:ft_deriv_mul}\usesdefs{A_def,bracket bump,normalized function tuples,auto:prism-form-definition,defn:window based bump functions}
Let $\gamma=\frac12$. For every $k\le-1$ and every increasing sequence of integers $(k_j)_{j\in[J)}$,
...
C_{\ref{lem:leftbump1_short2}}
<\tfrac{185801}{262144}2^{630}<\tfrac{23}{32}2^{630}<2^{630}.
\end{equation}
\end{lemma}
\begin{proof}[Proof \auto]
...
\begin{lemma}\label{lem:leftbump1_long}\uses{lem:bootstrap,A to Lambda,lem:theta_prim,P:induct-positive-terms-reduction-whitney,lem:phi4_supp,lem:form_pos}\usesdefs{A_def,bracket bump,normalized function tuples,auto:prism-form-definition,defn:window based bump functions}
...
C_{\ref{lem:leftbump1_long}}<\tfrac{1397}{2048}2^{625}<\tfrac{11}{16}2^{625}<2^{625}.
\end{equation}
\end{lemma}
\begin{proof}[Proof \auto]
