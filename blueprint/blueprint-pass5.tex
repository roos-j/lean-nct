...
\begin{center}
    ...
    This is the document of PASS 5.}}
\end{center}
...
\begin{theorem}[Main ergodic theorem \auto]\label{thm:ergodicthm}\using{thm:ergodicthmJ}\using{jump estimates imply variation}
...
\begin{theorem}[Main twisted theorem]\label{thm:nct main real}\using{lem:smoothingdecomp}\using{lem:norm_A_sum_le_sum}\using{lem:mainbump1}\using{lem:mainbump2}\using{lem:leftbump}\using{lem:leftbump1}\using{constant main bump one}\using{constant main bump two}\using{constant left bump}\using{constant left bump one}
...
C_{\ref{thm:nct main real}}=2^{630}.
\end{equation}
\end{theorem}
...
\end{equation}
where $C_{\ref{square root of Gaussian decay}}=19$.
\end{proposition}
\begin{proof}[Proof \auto]
Set
\[
...
|\widecheck B(x)|\le17\langle x\rangle^2.
\end{equation}
Combining these estimates, we obtain
\[
|\rho(x)|\le\tfrac{37}{2}\langle x\rangle^2
<19\langle x\rangle^2.
\]
...
C_{\ref{diagonal square root},2}=3648\sqrt2<2^{13}.
\end{equation}
\end{lemma}
\begin{proof}[Proof \auto]
...
C_{\ref{derivative of diagonal square root},2}=4608\sqrt{76}<2^{16}.
\end{equation}
\end{lemma}
\begin{proof}[Proof \auto]
By \eqref{auto:Gaussian-bump-decay-constant},
...
C_{\ref{Gaussian bump decay},0,2}=12,
\qquad
C_{\ref{Gaussian bump decay},1,2}=12\sqrt{76},
\qquad
C_{\ref{two bump estimate},2,2}=16.
\]
Substitution in the defining maximum proves the first estimate. At $N=2$, the product entry is the larger one and gives the stated exact value.
\end{proof}
...
C_{\ref{L:gaussian-bump-estimate},0}<24,\qquad
C_{\ref{L:gaussian-bump-estimate},1}<27,\qquad
C_{\ref{L:gaussian-bump-estimate},2}<35,\qquad
C_{\ref{L:gaussian-bump-estimate},3}<49.
\end{equation}
\end{lemma}
\begin{proof}[Proof \auto]
...
\]
The binomial coefficients in the definition of $C_{\ref{L:gaussian-bump-estimate},N}$ sum to $2^N$, and $(2\pi)^{-N}\le1$, which proves the first estimate.

Since $e^{2\pi}<536<(579/25)^2$ and $\pi>157/50$, the exact formulas for $N=0,1,2,3$ give, respectively,
\[
e^\pi,
\qquad
...
\]
Substitution gives the four displayed bounds.\jr{GPT: Another example of constant waste. When aboslute constants are $<1000$ you don't have to use a power of $2$, instead use the smaller number \cgpt}\jr{GPT: On second thought, we might be making formalization unnecessarily difficult here with these constant evaluations. Please here and in the rest of the manuscript, simplify estimates of expressions involving universal constants as follows: use only the estimate $2<e<3$ for $e$ and the estimate $3<\pi<4$ for $\pi$. If this makes some constants a bit larger, that is acceptable.}
\end{proof}
...
\begin{lemma}[constant $C_{\ref{L:faa-di-bruno},N}$ \auto]\label{constant faa di bruno}\using{L:faa-di-bruno}\using{L:derivative-estimate-for-G}\using{Gaussian bump decay}
For every $N\in\N$,
...
C_{\ref{L:faa-di-bruno},0}\le54,\qquad
C_{\ref{L:faa-di-bruno},1}<\tfrac{45}{64}2^{14},\qquad
C_{\ref{L:faa-di-bruno},2}<\tfrac{529}{1024}2^{32},\qquad
C_{\ref{L:faa-di-bruno},3}<\tfrac{521}{1024}2^{56}.
\end{equation}
\end{lemma}
\begin{proof}[Proof \auto]
Since $\pi>157/50$, we have $3\pi/16>471/800$. The first five terms of the exponential series give
\[
e^{471/800}>\frac95.
\]
Consequently,
...
1-e^{-3\pi/16}>\frac49,
\]
and Lemma \ref{L:derivative-estimate-for-G} gives
\[
C_{\ref{L:derivative-estimate-for-G},q}
\le q!\left(\tfrac94\right)^{q+1}.
\]
Together with \eqref{auto:Gaussian-bump-decay-constant}, the fact that a partition of $[N)$ has at most $N$ blocks, and the bound $N^N$ for the number of partitions, this proves the first estimate by substitution in the defining sum.

For $N=0$, the defining formula gives $C_{\ref{L:faa-di-bruno},0}\le54$. For $N=1$, it gives
\[
...
=\tfrac{45}{64}2^{14},
\]
where $\sqrt{76}<80/9$. For $N=2$, direct substitution gives
\[
...
\sqrt{19}<\tfrac{109}{25},
\qquad
\sqrt{114}<\tfrac{267}{25}
\]
in the five partitions of $[3)$. The defining sum is then smaller than
\[
\tfrac{183230359232099328}{5}
<\tfrac{521}{1024}2^{56}.
\]
\end{proof}
...
\begin{lemma}[constant $C_{\ref{L:second-gaussian-estimate},N}$ \auto]\label{constant second gaussian estimate}\using{L:gaussian-bump-estimate}\using{L:faa-di-bruno}\using{constant gaussian bump estimate}\using{constant faa di bruno}
For every $N\in\N$,
...
C_{\ref{L:second-gaussian-estimate},0}<\tfrac{313}{512}2^{14}<2^{14},
\qquad
C_{\ref{L:second-gaussian-estimate},1}<\tfrac{43}{64}2^{19}<2^{19},
\end{equation}
\begin{equation}\label{auto:constant-second-gaussian-estimate-small-high}
C_{\ref{L:second-gaussian-estimate},2}<\tfrac{39}{64}2^{34}<2^{34},
\qquad
C_{\ref{L:second-gaussian-estimate},3}<\tfrac{49}{128}2^{56}<2^{56}.
\end{equation}
\end{lemma}
\begin{proof}[Proof \auto]
Substitute the estimates from the defining formulas of Lemmas \ref{L:gaussian-bump-estimate} and \ref{L:faa-di-bruno} into the defining sum. Bounding the $N+1$ summands and the binomial coefficients gives the first estimate. For $N=0,1,2,3$, use $e^\pi<579/25$, $\pi>157/50$, the four exact formulas in the proof of Lemma \ref{constant gaussian bump estimate}, and the finite estimates in Lemma \ref{constant faa di bruno}. Direct substitution in the four finite sums gives the displayed estimates.
\end{proof}
...
C_{\ref{four scale Gaussian kernel},2}<\tfrac{313}{512}2^{36},
\qquad
C_{\ref{four scale Gaussian kernel},3}<\tfrac{99}{128}2^{58}.
\end{equation}
\end{lemma}
\begin{proof}[Proof \auto]
Substitute $C_{\ref{lem:smoothdecay2},N}=2^N$ and the defining estimates from Lemmas \ref{L:gaussian-bump-estimate} and \ref{L:second-gaussian-estimate}. This proves the first estimate. For $N=2$, use
\[
C_{\ref{L:gaussian-bump-estimate},2}<35,
\qquad
C_{\ref{L:second-gaussian-estimate},2}<\tfrac{39}{64}2^{34}.
\]
For $N=3$, use
\[
C_{\ref{L:gaussian-bump-estimate},3}<49,
\qquad
C_{\ref{L:second-gaussian-estimate},3}<\tfrac{49}{128}2^{56}.
\]
Substitution in \eqref{auto:four-scale-Gaussian-kernel-constant} gives the displayed bounds.
\end{proof}
...
C_{\ref{mean four scale Gaussian kernel},2}<\tfrac{313}{512}2^{41}.
\end{equation}
\end{lemma}
\begin{proof}[Proof \auto]
Substitution of the defining estimates from Lemmas \ref{L:gaussian-bump-estimate} and \ref{L:second-gaussian-estimate} proves the first bound. For $N=2$, use the finite estimates in Lemmas \ref{constant gaussian bump estimate} and \ref{constant second gaussian estimate}. In the second maximum in \eqref{auto:mean-four-scale-Gaussian-constant}, the term with $l=2$ is the largest. Substitution gives the displayed estimate.
\end{proof}
...
C_{\ref{H kernel derivative estimate Gaussian domination}}
<\tfrac{99}{128}2^{30}<2^{30}.
\end{equation}
\end{lemma}
\begin{proof}[Proof \auto]
By \eqref{auto:constant-diagonal-square-root-two} and \eqref{auto:constant-derivative-diagonal-square-root-two},
\[
C_{\ref{diagonal square root},2}=3648\sqrt2,
\qquad
C_{\ref{derivative of diagonal square root},2}=4608\sqrt{76}.
\]
Also $C_{\ref{Gaussian bump decay},0,2}=12$ and
$C_{\ref{Gaussian bump decay},1,2}=12\sqrt{76}$. Using $\sqrt{152}<37/3$ and $\sqrt{76}<9$, the defining sum is smaller than
\[
4\cdot3648\cdot4608\cdot\tfrac{37}{3}+2\cdot12^2\cdot9
<\tfrac{99}{128}2^{30}.
\]
\end{proof}


\begin{proposition}[Gaussian domination, combined]\label{Gaussian domination combined}\using{Gauss domination case 1}\using{Gauss domination case 2}\using{Gauss domination case 3}\using{Gauss domination constant}\jr{GPT: Put the following three constant definitions in one line, similar elsewhere \cgpt}\jr{GPT: in the definition of $C_{\ref{Gaussian domination combined},2}$ you are violating the rule that absolute constants should be estimated by powers of $2$ if they are $\ge 1000$, please fix that here and throughout (even if that means the constants get slightly larger). Make sure you don't forget to propagate constant changes throughout. There are many occurrences of the number $1970001$ below, please fix that, we don't want such larger numbers coming up.}
Set
...
C_{\ref{Gaussian domination combined},2}=1970001\cdot2^{123}.
...
C_{\ref{H kernel estimate Gaussian domination}}<\tfrac{51}{64}2^{27},
\end{equation}
and
\begin{equation}\label{auto:H-kernel-derivative-small-constant-for-Gaussian-domination}
C_{\ref{H kernel derivative estimate Gaussian domination}}<\tfrac{99}{128}2^{30}.
\end{equation}
Furthermore,
...
C_{\ref{two bump estimate},2,2}=16.
\end{equation}
The bump triangle constants in Propositions \ref{Gauss domination case 1}, \ref{Gauss domination case 2}, and \ref{Gauss domination case 3} are respectively $1,4,4$. Thus the maximum in \eqref{auto:Gaussian-domination-case-one-constant} is smaller than $68$. Since $\pi<22/7$ and $e^{2\pi}<536$, substitution gives
\[
C_{\ref{Gauss domination case 1}}
<2^7\tfrac{22}{7}\cdot536\cdot2^{18}
\cdot\tfrac{313}{512}2^{41}
\cdot\tfrac{51}{64}2^{27}\cdot68
<2^{109},
\]
\[
C_{\ref{Gauss domination case 2}}
<3\cdot2^7\cdot536\cdot2^{33}
\cdot\tfrac{99}{128}2^{58}
\cdot\tfrac{99}{128}2^{30}\cdot4\cdot16
=1970001\cdot2^{123},
\]
and
\[
C_{\ref{Gauss domination case 3}}
<2^7\cdot536\cdot2^{18}
\cdot\tfrac{313}{512}2^{36}
\cdot\tfrac{51}{64}2^{27}\cdot4\cdot16
<2^{103}.
\]
\end{proof}
...
C_{\ref{induct positive terms imply increase data}}<2^{162}.
\end{equation}
\end{lemma}
\begin{proof}[Proof \auto]
Using \eqref{Gaussian domination combined constants}, the defining product is
\[
2^{10}\cdot28\cdot9\cdot1970001\cdot2^{123}
=252\cdot1970001\cdot2^{133}
<2^{162}.
\]
\end{proof}
...
(1970001\cdot2^{143}\sqrt{252}(k+2)+2^{1/2})^2}
\end{equation}
holds.
\end{lemma}
\begin{proof}[Proof \auto]
...
C_k\le
\bigl(1970001\cdot2^{143}\sqrt{252}(k+2)+2^{1/2}\bigr)^2.
\end{equation}
...
=1970001\cdot2^{143}\sqrt{252},
\]
Proposition \ref{P:C_k-induction} gives
\[
C_k
\le
2+k\cdot1970001\cdot2^{143}\sqrt{252}
\bigl(1970001\cdot2^{143}\sqrt{252}(k+3)+2^{1/2}\bigr).
\]
...
C_{\ref{induct positive terms theorem}}<\tfrac78 2^{340}<2^{340}.
\end{equation}
\end{lemma}
\begin{proof}[Proof \auto]
The defining value of $C_{\ref{Gaussian domination combined},2}$ is $1970001\cdot2^{123}$. Since
\[
C_{\ref{induct positive terms imply increase data}}
=2^{10}\cdot252\,C_{\ref{Gaussian domination combined},2},
\]
the first summand in \eqref{auto:positive-induction-constant-definition} is
\[
1970001\cdot2^{145}\sqrt{252}.
\]
Its square is smaller than $\frac{223}{256}2^{340}$. The remaining two terms in the expanded square are smaller than $2^{174}+2$, and hence smaller than $\frac1{256}2^{340}$. Therefore
\[
C_{\ref{induct positive terms theorem}}
<\tfrac78 2^{340}<2^{340}.
\]
...
1+|v_i|\le3(1+|v_i-tR|).
\]
Consequently the left-hand side of \eqref{E:two-terms} is at most
\[
81R^{-1/2}\langle v_0\rangle^{3/2}\langle v_1\rangle^{3/2}
\le81\langle v_0\rangle^{3/2}\langle v_1\rangle^{3/2}.
\]
...
\begin{definition}[Universal pair]\label{def:unipair}\jr{GPT: The constants $N_0$, $C_0$ defined here should be renamed to $N_{\ref{def:unipair}}$ and $C_{\ref{def:unipair}}$, respectively. Please do this refactoring throughout}
Set
...
\le2^{q(q-1)/2+3q}
\]
for $1\le q\le3$. Applying the Fa\`a di Bruno formula to the two trigonometric compositions, using $\pi/2<11/7$, and grouping terms by their block sizes gives the following bounds for derivatives of orders $1,2,3$:
\[
2^4,
\qquad
2^9,
...
\]
Direct arithmetic shows that these are smaller than the three displayed powers of two. Thus every derivative of order at most $N_0$ is bounded by $C_0$. The order-zero bound is at most $1$, so $(\phi_0,\phi_1)$ is a $(C_0,N_0)$-pair.
\end{proof}
...
C_{\ref{lem:theta_decay},N}\le2^{2N+16}.
\end{equation}
\end{lemma}
\begin{proof}[Proof \auto]
Substitute $C_0=2^{14}$ in \eqref{auto:theta-decay-constant-definition}.
\end{proof}
...
C_{\ref{lem:abs_deriv_ft_phi3_le},1}=28C_0+4,
\end{equation}
for $m=2$,
\begin{equation}\label{auto:varphi-three-Fourier-derivative-constant-two}
C_{\ref{lem:abs_deriv_ft_phi3_le},2}=96C_0^2+140C_0+53,
\end{equation}
and, for $m=3$,
\begin{equation}\label{auto:varphi-three-Fourier-derivative-constant-three}
C_{\ref{lem:abs_deriv_ft_phi3_le},3}=1104C_0^2+1657C_0+993.
\end{equation}
\end{lemma}
\begin{proof}[Proof \auto]
...
\]
For $q=1,2,3$, respectively, this is bounded by
\[
4,
\qquad
53,
...
\begin{lemma}[constant $C_{\ref{lem:abs_deriv_ft_phi3_le},m}$ \auto]\label{constant phi three derivative}\using{lem:abs_deriv_ft_phi3_le}
For $m=0$,
...
C_{\ref{lem:abs_deriv_ft_phi3_le},1}<2^{19}.
\end{equation}
For $m=2$,
\begin{equation}\label{auto:constant-phi-three-derivative-two}
C_{\ref{lem:abs_deriv_ft_phi3_le},2}<2^{35}.
\end{equation}
For $m=3$,
\begin{equation}\label{auto:constant-phi-three-derivative-three}
C_{\ref{lem:abs_deriv_ft_phi3_le},3}<2^{39}.
\end{equation}
\end{lemma}
\begin{proof}[Proof \auto]
Substitute $C_0=2^{14}$ in the four defining values.
\end{proof}
...
\begin{lemma}[constant $C_{\ref{lem:abs_deriv_ft_Tphi3_le},m}$ \auto]\label{constant T phi three derivative}\using{lem:abs_deriv_ft_Tphi3_le}
For $m=0$,
\begin{equation}\label{constant T phi three derivative bound}
C_{\ref{lem:abs_deriv_ft_Tphi3_le},0}<2^{19}.
\end{equation}
For $m=1$,
\begin{equation}\label{auto:constant-T-phi-three-derivative-one}
C_{\ref{lem:abs_deriv_ft_Tphi3_le},1}<2^{35}.
\end{equation}
For $m=2$,
\begin{equation}\label{auto:constant-T-phi-three-derivative-two}
C_{\ref{lem:abs_deriv_ft_Tphi3_le},2}<2^{39}.
\end{equation}
\end{lemma}
\begin{proof}[Proof \auto]
Substitute $C_0=2^{14}$ in the three exact values from Lemma \ref{lem:abs_deriv_ft_Tphi3_le}.
\end{proof}
...
$\|\widehat\theta^{(m-q)}\|_\infty\le2^{m-q+1}C_0$. For $m=0,1,2,3$, direct substitution in this finite sum gives upper bounds
\[
2^3C_0,
\qquad
2^6C_0,
...
\]
for $2\le N<N_0$. This proves \eqref{thetaprim_decay} and \eqref{Thetaprim_decay}.
\end{proof}
...
C_{\ref{lem:theta_prim},N}\le2^{5N+20}.
\end{equation}
In particular,
\begin{equation}\label{auto:constant-theta-primitive-two}
C_{\ref{lem:theta_prim},2}\le2^{30}.
\end{equation}
\end{lemma}
\begin{proof}[Proof \auto]
Substitute $C_0=2^{14}$ in \eqref{auto:theta-primitive-constant-definition}.
\end{proof}
...
\begin{lemma}\label{lem:form_pos}\using{M to K}\using{prism BL inequality}
...
C_{\ref{lem:Phij_prop}}=2^{26}.
\end{equation}
\end{lemma}
\begin{proof}[Proof \auto]
This follows from \eqref{auto:Phij-constant-definition} and $C_0=2^{14}$.
\end{proof}
...
\begin{lemma}[constant $C_{\ref{lem:rho-kernels-reduction}}$ \auto]\label{constant rho kernels reduction}\using{lem:rho-kernels-reduction}\using{constant mean four scale Gaussian kernel}\using{constant four scale Gaussian kernel}\using{mean four scale Gaussian kernel}\using{four scale Gaussian kernel}
\begin{equation}\label{constant rho kernels reduction bound}
C_{\ref{lem:rho-kernels-reduction}}
<\tfrac{10329}{16384}2^{62}<2^{62}.
\end{equation}
\end{lemma}
\begin{proof}[Proof \auto]
...
\ref{Properties of distance of sequences},\jr{GPT: Here you used aligned environment. You are not allowed to use this or other multiline environments. Use simple multiple single line displays instead.}
\[
\begin{aligned}
\Delta(p_{h,b,m})
&=
...
\begin{lemma}[constant $C_{\ref{lem:increase-data-Gaussian-expansion}}$ \auto]\label{constant increase data Gaussian expansion}\using{lem:increase-data-bracket-domination}\using{constant increase data bracket domination}\using{Gaussian domination}\using{lem:increase-data-Gaussian-expansion}
\begin{equation}\label{constant increase data Gaussian expansion bound}
C_{\ref{lem:increase-data-Gaussian-expansion}}
<\tfrac{85}{128}2^{84}<2^{84}.
\end{equation}
\end{lemma}
\begin{proof}[Proof \auto]
Since $C_{\ref{Gaussian domination}}=e^\pi$ and $e^{2\pi}<536$, the defining product and Lemma \ref{constant increase data bracket domination} give
\[
C_{\ref{lem:increase-data-Gaussian-expansion}}
<2^3\cdot536\cdot\tfrac{10329}{16384}2^{72}
=\tfrac{692043}{1048576}2^{84}
<\tfrac{85}{128}2^{84}.
\]
\end{proof}
...
\[
C_{\ref{P:increase-data-reduction}}
<\tfrac78\tfrac{85}{128}2^{22+340+84+6}
=\tfrac{595}{1024}2^{452}.
\]
\end{proof}
...
C_{\ref{P:one-scale-estimate-window}}=2^{37}.
\end{equation}
\end{lemma}
\begin{proof}[Proof \auto]
Substitute $C_0=2^{14}$ in the defining value $2^9C_0^2$.
\end{proof}
...
<\tfrac{25}{64}2^{511}<2^{511}.
\end{equation}
\end{lemma}
\begin{proof}[Proof \auto]
Use Lemma \ref{auto:constant-one-scale-window}, $C_0=2^{14}$, $C_{\ref{lem:smoothdecay2},2}=2^2$, and Lemma \ref{constant diagonal band reduction}. Since $\pi>157/50$, substitution in the defining sum gives
\[
C_{\ref{P:induct-positive-terms-reduction-non-whitney}}
<2^{37}
+2^{10}\left(\tfrac{2^{14}}{2\pi}\right)^4
\tfrac{149}{256}2^{455}
<\tfrac{25}{64}2^{511}.
\]
\end{proof}
...
\tfrac{25}{64}2^{513}.
\]
Lemmas \ref{constant Phij proposition} and \ref{constant diagonal band reduction}, together with $C_0=2^{14}$, bound the second term by
\[
2^{19}(2^{28}+2^{52})\tfrac{149}{256}2^{455}.
\]
The sum of these two bounds is smaller than $\frac{75}{128}2^{526}$.
\end{proof}
...
\label{P:induct-positive-terms-reduction-whitney-product}\using{P:induct-positive-terms-reduction-whitney}
Let $a\in A$ and let
...
\end{equation}
\jr{GPT: If possible, Instead of this assumption, use assumption {auto:Whitney-product-Fourier-derivative-assumption} and Lemma \ref{lem:smoothdecay2} in the proof. Update constants and use sites accordingly. If you end up not doing this, you must explain why not in your report.}
\begin{equation}\label{Whitney product decay assumption}
|\psi(x)|\le\langle x\rangle^2
\end{equation}
...
=4C_{\ref{P:induct-positive-terms-reduction-whitney}}.
\end{equation}
\end{proposition}

\begin{proof}[Proof \auto]
Set $\Psi=2^{-2}\psi\otimes\psi$. Then $\Psi$ is symmetric and satisfies \eqref{p1-on-W_pos} and \eqref{p1-on-W_supp}. By \eqref{auto:Whitney-product-Fourier-derivative-assumption}, the zeroth, first, and second derivatives of $\widehat\psi(\xi)\widehat\psi(-\xi)$ are bounded by $1$, $2$, and $4$, respectively, so \eqref{p1-on-W_der} holds. Moreover,
\[
|\Psi(v)|
\le2^{-2}\langle v_0\rangle^2\langle v_1\rangle^2
\le\langle v_0\rangle^{3/2}\langle v_1\rangle^{3/2},
\]
so \eqref{p1-on-W_decay} holds. Proposition \ref{P:induct-positive-terms-reduction-whitney} applied to $2^{-2}M_j$ proves the claim.
\end{proof}
...
\]
Apply Proposition \ref{P:induct-positive-terms-reduction-whitney-product} with $2^{-2}\psi_{(t)}$ in place of $\psi$, and multiply the resulting estimate by $2^4$. The Fourier support condition follows from \eqref{main_aux1_supp} and $t\in[1,2]$. Moreover,
\[
2^{-2}|\psi_{(t)}(u)|
\le2^{-2}t^{-1}\langle t^{-1}u\rangle^2
\le\langle u\rangle^2,
\]
so \eqref{Whitney product decay assumption} holds. Finally,
\[
...
C_{\ref{lem:main_aux1}}<\tfrac{825}{1024}2^{535}<2^{535}.
\end{equation}
\end{lemma}
\begin{proof}[Proof \auto]
...
Assume that the function $T\phi(s) = (s \phi(s))'$ satisfies \eqref{main_aux1_supp}, \eqref{main_aux1_decay}, and \eqref{auto:main-auxiliary-one-Fourier-derivative-assumption}.
...
\]
For each fixed $t\in [1,2]$ we use Lemma \ref{lem:main_aux1} (as $T\phi$ satisfies all three assumptions of Lemma \ref{lem:main_aux1}) and that $\#\kappa\le J+1$ to obtain
...
=(4C_0C_{\ref{lem:smoothdecay2},2})^2C_{\ref{lem:main_aux1}}.
\end{equation}
\end{lemma}
...
\[
c_1=4C_0C_{\ref{lem:smoothdecay2},2}.
\]
The Fourier transform of $\phi_0-\phi_1$ is supported in $\mathrm{Ann}_1(1,2)$. By \eqref{windowbound}, its derivatives through order $2$ have modulus at most $2C_0$. Lemma \ref{lem:smoothdecay2}, applied with $N=2$, gives
\[
|\phi_0(u)-\phi_1(u)|
\le4C_0C_{\ref{lem:smoothdecay2},2}\langle u\rangle^2.
\]
After division by $c_1$, the Fourier derivatives through order $2$ are bounded by $1$. Thus all three assumptions of Lemma \ref{lem:main_aux1} hold, and the result follows.
\end{proof}
...
C_{\ref{lem:mainbump1_long1}}<\tfrac{825}{1024}2^{571}<2^{571}.
\end{equation}
\end{lemma}
\begin{proof}[Proof \auto]
Use $C_0=2^{14}$, $C_{\ref{lem:smoothdecay2},2}=2^2$, and Lemma \ref{constant main auxiliary one} in \eqref{auto:main-bump-one-long-one-constant-definition}. The squared factor is $2^{36}$, so
\[
C_{\ref{lem:mainbump1_long1}}
<\tfrac{825}{1024}2^{571}.
\]
...
C_{\ref{lem:mainbump1_long2}}<\tfrac{25}{64}2^{513}<2^{513}.
\end{equation}
\end{lemma}
\begin{proof}[Proof \auto]
...
C_{\ref{lem:mainbump1_long}}<\tfrac{413}{512}2^{572}<2^{572}.
\end{equation}
\end{lemma}
\begin{proof}[Proof \auto]
Use Lemmas \ref{constant main bump one long one} and \ref{constant main bump one long two} in the defining sum. The first contribution after multiplication by $2$ is smaller than $\frac{825}{1024}2^{572}$, while the second is smaller than $\frac1{1024}2^{572}$. Their sum gives the displayed estimate.
\end{proof}
...
=2^{10}C_0^2C_{\ref{lem:smoothdecay2},2}^2C_{\ref{lem:main_aux1}}
...
Let
\[
c_1=2^3C_0C_{\ref{lem:smoothdecay2},2}.
\]
...
\]
The long variation assumption follows from Lemma \ref{lem:mainbump1_long}. It remains to verify the three assumptions on $c_1^{-1}T\phi_0$.

By Lemma \ref{lem:ft_deriv_mul}, $\widehat{T\phi_0}$ is supported in $\mathrm{Ann}_1(1,2)$ and
\[
\|(\widehat{T\phi_0})^{(2)}\|_\infty
\le2\|\widehat{\phi_0}^{(2)}\|_\infty
+\|\widehat{\phi_0}^{(3)}\|_\infty
\le3C_0.
\]
Lemma \ref{lem:smoothdecay2} with $N=2$ gives
\[
|T\phi_0(u)|
\le6C_0C_{\ref{lem:smoothdecay2},2}\langle u\rangle^2
\le c_1\langle u\rangle^2.
\]
The identity \eqref{auto:Fourier-derivatives-of-T} similarly bounds the Fourier derivatives of $\widehat{T\phi_0}$ through order $2$ by $3C_0$, hence by $c_1$ after normalization. Lemma \ref{lem:shortlongftc_reduction} now gives the stated constant.
\end{proof}
...
C_{\ref{lem:mainbump1}}<\tfrac78 2^{577}<2^{577}.
\end{equation}
\end{lemma}
\begin{proof}[Proof \auto]
Use $C_0=2^{14}$, $C_{\ref{lem:smoothdecay2},2}=2^2$, and Lemma \ref{constant main auxiliary one} in the first term of \eqref{auto:main-bump-one-constant-definition}. It is smaller than
\[
\tfrac{825}{1024}2^{577}.
\]
...
\tfrac{413}{512}2^{573}.
\]
The sum is smaller than $\frac78 2^{577}$.
\end{proof}

\begin{lemma}\label{lem:main_aux2}\using{lem:shortlongftc_reduction}\using{lem:main_aux1}\using{lem:bootstrap}
Let $\psi$ satisfy \eqref{main_aux1_supp}, \eqref{main_aux1_decay}, and \eqref{auto:main-auxiliary-one-Fourier-derivative-assumption}. Assume also that
\begin{equation}\label{main_aux2_decay}
|T\psi(u)|\le\langle u\rangle^2
\end{equation}
for all $u\in\R$, and
...
\begin{proof}
We apply Lemma \ref{lem:shortlongftc_reduction} with $\phi=\psi$. The support of $\widehat{T\psi}$ is contained in the support of $\widehat\psi$, and the other two assumptions on $T\psi$ are \eqref{main_aux2_decay} and \eqref{auto:main-auxiliary-two-Fourier-derivative-assumption}.
...
\]
Lemma \ref{lem:main_aux1}, applied with $t=1$, gives \eqref{eq:main_aux2_long} because $\psi$ satisfies all three of its assumptions.
\end{proof}
...
C_{\ref{lem:main_aux2}}<\tfrac{2475}{4096}2^{540}<2^{540}.
\end{equation}
\end{lemma}
\begin{proof}[Proof \auto]
...
<24\cdot\tfrac{825}{1024}2^{535}
=\tfrac{2475}{4096}2^{540}.
\]
\end{proof}
...
C_{\ref{lem:abs_deriv_ft_Tphi3_le},2}^2
C_{\ref{lem:smoothdecay2},2}^2.
\end{equation}
\end{lemma}
...
\[
c_1=2C_{\ref{lem:abs_deriv_ft_Tphi3_le},2}C_{\ref{lem:smoothdecay2},2}.
\]
By Lemma \ref{lem:ft_phi3_eq},
...
\end{equation}
Lemma \ref{lem:abs_deriv_ft_phi3_le} with $m=2$, followed by Lemma \ref{lem:smoothdecay2} with $N=2$, gives
\begin{equation}\label{mainbump2_dec1}
|\varphi_{3,k}(u)|
\le2C_{\ref{lem:abs_deriv_ft_Tphi3_le},2}C_{\ref{lem:smoothdecay2},2}\langle u\rangle^2.
\end{equation}
Similarly, Lemma \ref{lem:abs_deriv_ft_Tphi3_le} with $m=2$ gives
\begin{equation}\label{mainbump2_dec2}
|T\varphi_{3,k}(u)|
\le2C_{\ref{lem:abs_deriv_ft_Tphi3_le},2}C_{\ref{lem:smoothdecay2},2}\langle u\rangle^2.
\end{equation}
The support of $\widehat{T\varphi_{3,k}}$ is contained in the same annulus by Lemma \ref{lem:ft_deriv_mul}. Lemmas \ref{lem:abs_deriv_ft_phi3_le} and \ref{lem:abs_deriv_ft_Tphi3_le} also show that, after division by $c_1$, the Fourier derivatives of both functions through order $2$ are bounded by $1$. Thus all assumptions of Lemma \ref{lem:main_aux2} hold.
\end{proof}
...
C_{\ref{lem:mainbump2}}<\tfrac{31}{64}2^{623}<2^{623}.
\end{equation}
\end{lemma}
\begin{proof}[Proof \auto]
Use Lemma \ref{constant main auxiliary two}, $C_{\ref{lem:smoothdecay2},2}=2^2$, and the exact value
\[
C_{\ref{lem:abs_deriv_ft_Tphi3_le},2}
=1296C_0^2+1937C_0+1099
\]
with $C_0=2^{14}$ in \eqref{auto:main-bump-two-constant-definition}. Direct substitution gives
\[
C_{\ref{lem:mainbump2}}
<\tfrac{31}{64}2^{623}.
\]
...
C_{\ref{lem:leftbump}}
=C_{\ref{lem:main_aux2}}C_{\ref{lem:theta_prim},2}^2.
\end{equation}
\end{lemma}
...
\]
By Lemma \ref{lem:rescaling}, it suffices to apply Lemma \ref{lem:main_aux2} to $C_{\ref{lem:theta_prim},2}^{-1}\widetilde\theta$. The support and decay assumptions for this function and its image under $T$ are \eqref{thetaprim_supp}, \eqref{Tthetaprim_supp}, \eqref{thetaprim_decay}, and \eqref{Thetaprim_decay} with $N=2$. The finite Fourier-derivative estimates in the proof of Lemma \ref{lem:theta_prim}, together with
\[
(\widehat{T\widetilde\theta})^{(m)}
=-\xi\widehat{\widetilde\theta}^{(m+1)}
-m\widehat{\widetilde\theta}^{(m)},
\]
verify the two Fourier-derivative assumptions through order $2$ after the same normalization.
\end{proof}
...
C_{\ref{lem:leftbump}}<\tfrac{2475}{4096}2^{600}<2^{600}.
\end{equation}
\end{lemma}
\begin{proof}[Proof \auto]
Use Lemmas \ref{constant main auxiliary two} and \ref{constant theta primitive}, with $N=2$, in \eqref{auto:left-bump-constant-definition}. This gives the displayed estimate.
\end{proof}

\begin{lemma}\label{lem:leftbump1_short1}\using{A to Lambda}\using{lem:theta_prim}\using{lem:thetat_offcenter}\using{P:induct-positive-terms-reduction-whitney}\using{lem:int_fct}\using{lem:phi4_supp}
...
\begin{lemma}[constant $C_{\ref{lem:leftbump1_short1}}$ \auto]\label{constant left bump one short one}\using{constant Whitney reduction}\using{constant theta primitive}\using{constant off center bump}\using{lem:leftbump1_short1}
\begin{equation}\label{constant left bump one short one bound}
C_{\ref{lem:leftbump1_short1}}<\tfrac{27}{32}2^{598}<2^{598}.
\end{equation}
\end{lemma}
\begin{proof}[Proof \auto]
...
\[
C_{\ref{lem:leftbump1_short1}}
<\tfrac{109725}{131072}2^{598}
<\tfrac{27}{32}2^{598}.
\]
\end{proof}


\begin{lemma}\label{lem:leftbump1_short2}\using{A to Lambda}\using{lem:theta_prim}\using{lem:theta_decay}\using{lem:thetat_offcenter}\using{P:induct-positive-terms-reduction-whitney}\using{lem:int_fct}\using{lem:phi4_supp}\using{lem:ft_deriv_mul}
Let $\gamma=\frac12$. For every $k\le-1$ and every increasing sequence of integers $(k_j)_{j\in[J)}$,
...
\begin{lemma}[constant $C_{\ref{lem:leftbump1_short2}}$ \auto]\label{constant left bump one short two}\using{lem:theta_prim}\using{constant theta primitive}\using{constant theta decay}\using{constant Whitney reduction}\using{constant off center bump}\using{lem:leftbump1_short2}
\begin{equation}\label{constant left bump one short two bound}
C_{\ref{lem:leftbump1_short2}}<\tfrac{27}{32}2^{600}<2^{600}.
\end{equation}
\end{lemma}
\begin{proof}[Proof \auto]
Lemmas \ref{constant theta primitive} and \ref{constant theta decay} give
\[
C\le C_{\ref{lem:theta_prim},2}\le2^{30}.
\]
...
\[
C_{\ref{lem:leftbump1_short2}}
<\tfrac{109725}{131072}2^{600}
<\tfrac{27}{32}2^{600}.
\]
\end{proof}


\begin{lemma}\label{lem:leftbump1_long}\using{lem:bootstrap}\using{A to Lambda}\using{lem:theta_prim}\using{P:induct-positive-terms-reduction-whitney}\using{lem:phi4_supp}\using{lem:form_pos}
...
C_{\ref{lem:leftbump1_long}}<\tfrac{825}{1024}2^{595}<2^{595}.
\end{equation}
\end{lemma}
\begin{proof}[Proof \auto]
...
\begin{lemma}[constant $C_{\ref{lem:leftbump1}}$ \auto]\label{constant left bump one}\using{constant left bump one short one}\using{constant left bump one short two}\using{constant left bump one long}\using{lem:leftbump1}
\begin{equation}\label{constant left bump one bound}
C_{\ref{lem:leftbump1}}<\tfrac{27}{32}2^{606}<2^{606}.
\end{equation}
\end{lemma}
\begin{proof}[Proof \auto]
...
\[
\tfrac{109725}{131072}2^{606}.
\]
Lemma \ref{constant left bump one long} bounds the second term by
\[
\tfrac{825}{1048576}2^{606}.
\]
Their sum is smaller than $\frac{27}{32}2^{606}$.
\end{proof}
